...
**Compiler note**: `AGIJobManager.sol` declares `pragma solidity ^0.8.26`, while the Truffle default compiler is `0.8.26` (configurable via `SOLC_VERSION`). `viaIR` is enabled by default because compiling without it triggers stack‑too‑deep errors; keep compiler settings consistent for verification.
...
- `viaIR`: **true** (via `SOLC_VIA_IR`, required for stack‑too‑deep mitigation)
