...
**Compiler note**: `AGIJobManager.sol` declares `pragma solidity ^0.8.28`, while the Truffle default compiler is `0.8.28` (configurable via `SOLC_VERSION`). `viaIR` is **disabled by default**; enable it only if you encounter stack‑too‑deep errors and keep compiler settings consistent for verification.
...
- `viaIR`: **false** by default (enable only if needed to compile)
