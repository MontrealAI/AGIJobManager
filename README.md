...
**Compiler note**: `AGIJobManager.sol` declares `pragma solidity ^0.8.24`, while the Truffle default compiler is `0.8.24` (configurable via `SOLC_VERSION`). Keep the deploy-time compiler settings consistent for verification.
...
- [Minimal governance](docs/minimal-governance.md)
...
- `viaIR`: **true** (via `SOLC_VIA_IR`, required to avoid stack-too-deep errors in the non-IR pipeline)
