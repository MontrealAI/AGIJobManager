...
test('review recovery requires an existing address and defaults to read-only',()=>{
 const env={REVIEW_ESCROW_ADDRESS:'0x'+'3'.repeat(40),JOB_MANAGER:'0x'+'1'.repeat(40),JOB_MANAGER_CODE_HASH:'0x'+'2'.repeat(64)};
 assert.equal(settings(env).dryRun,true);assert.throws(()=>settings({...env,DRY_RUN:'nope'}));assert.throws(()=>settings({...env,DRY_RUN:'0'}),/VERIFY_RPC_URL/);assert.throws(()=>settings({...env,CONFIRMATIONS:'1'}));assert.throws(()=>settings({}),/New review-escrow deployment is disabled/);assert.throws(()=>settings({...env,REVIEW_ESCROW_ADDRESS:''}),/disabled/);
});
