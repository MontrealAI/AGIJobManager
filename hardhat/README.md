...
Default final owner used everywhere unless overridden:
...
- `FINAL_OWNER`: optional override; defaults to `0xa9eD0539c2fbc5C6BC15a2E168bd9BCd07c01201`.
...
# Edit deploy.config.local.js -> sepolia profile with real agiTokenAddress, ENS addresses, roots, and finalOwner.
npx hardhat compile
...
Use defaults directly from `deploy.config.example.js` and `.env.example`.

```bash
cd hardhat && npm ci
cp .env.example .env
# Set MAINNET_RPC_URL, PRIVATE_KEY, ETHERSCAN_API_KEY
DRY_RUN=1 npm run deploy:mainnet
DEPLOY_CONFIRM_MAINNET=I_UNDERSTAND_MAINNET_DEPLOYMENT npm run deploy:mainnet
```

If you need a different owner, override with `FINAL_OWNER=0x...` before the deploy command.
