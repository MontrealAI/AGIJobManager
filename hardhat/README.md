...
Review `JOB_MANAGER`, `ENS_DEPLOYMENT_MODE`, `JOBS_PARENT_NAME`, `ENS_REGISTRY`, `NAME_WRAPPER`, `PUBLIC_RESOLVER`, `NEW_OWNER`, `VERIFY` and `LOCK_CONFIG` before actual deployment. The default `fresh` mode derives `usdc-<chainId>-<full-lowercase-manager-address-without-0x>.<parent>` and sets the job prefix to `job-`. Mainnet defaults the parent to `alpha.jobs.agi.eth`; Sepolia requires an explicit reviewed `JOBS_PARENT_NAME`. Optional `JOBS_ROOT_NAME`, `JOBS_ROOT_NODE` and `JOB_LABEL_PREFIX` must match the derived values. The script rejects reused manager state and a root with existing ownership, resolver or TTL records, then repeats the namespace checks before deployment. Verify actual parent authority and recheck root availability before creating it. See the [namespace policy and historical deployment catalog](../docs/ENS/DEPLOYMENT_NAMESPACES.md). These current-source tools are newer than the frozen v1.0.0 downloads.

For a same-manager helper replacement, explicitly set `ENS_DEPLOYMENT_MODE=replacement` and `REPLACES_ENS_JOB_PAGES` to its current helper. Leave `JOBS_PARENT_NAME` unset. The script verifies the existing helper's manager/registry binding and preserves its root and default prefix. Historical job labels still require an exact inventory and any necessary imports; the script does not migrate them.
...
| Action | Signer |
| --- | --- |
| Deploy manager/libraries or ENSJobPages; configure helper manager and prefix | Deployer |
