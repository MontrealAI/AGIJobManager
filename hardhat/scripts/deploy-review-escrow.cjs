...
const {parseBooleanSetting,requireDeploymentNetwork,requireRuntimeSize,requireArtifactMatch,requireConfirmedReceipt,requireExplorerEnabled}=require('./deployment-safety.cjs');
function settings(env=process.env){
 if(!env.REVIEW_ESCROW_ADDRESS)throw Error('New review-escrow deployment is disabled. Recovery requires REVIEW_ESCROW_ADDRESS and the original deployment journal.');
 ...
 if(!dryRun&&!env.VERIFY_RPC_URL)throw Error('VERIFY_RPC_URL is required for verification recovery.');
 return {dryRun,confirmations,manager:env.JOB_MANAGER,managerCodeHash:env.JOB_MANAGER_CODE_HASH,existing:env.REVIEW_ESCROW_ADDRESS};
}
async function main(){
 ...
 if(config.dryRun){console.log(JSON.stringify({mode:'READ_ONLY_RECOVERY_PLAN',chainId,manager,token,managerCodeHash:config.managerCodeHash,existing:config.existing,terms:'Existing escrow source-verification recovery only. No deployment, funding or activation is performed.'},null,2));return;}
 ...
 journal.status='verified';journal.runtimeCodeHash=ethers.keccak256(deployedCode);journal.verification='verified';checkpoint();
 console.log(JSON.stringify({address,codeHash:journal.runtimeCodeHash,journal:file,status:'Existing escrow source verification recovered; no chain transactions sent.'},null,2));
 } finally { witness.destroy(); }
}
