...
function requireExplorerEnabled(config) {
  if (config?.verify?.etherscan?.enabled !== true) throw new Error('Etherscan verification must be enabled; a disabled verifier cannot establish deployment verification.');
  if (typeof config.verify.etherscan.apiKey !== 'string' || !config.verify.etherscan.apiKey.trim()) throw new Error('ETHERSCAN_API_KEY is required before deployment or verification.');
}
...
function resolveInputSource(buildInfo, userSource) {
  return buildInfo.userSourceNameMap?.[userSource] ||
...
function requireArtifactMatch({ artifact, buildInfo, address, libraries = {}, tokenAddress, code }) {
  ...
  for (const [source, names] of Object.entries(artifact.deployedLinkReferences || {})) {
    for (const [name, positions] of Object.entries(names)) {
      const linkedAddress = libraries[`${source}:${name}`] || Object.entries(libraries).find(([fqn]) => {
        const separator = fqn.lastIndexOf(':');
        return fqn.slice(separator + 1) === name && resolveInputSource(buildInfo, fqn.slice(0, separator)) === source;
      })?.[1];
  ...
  const target = buildInfo.output.contracts[artifact.inputSourceName || artifact.sourceName][artifact.contractName];
  const immutableReferences = target.evm.deployedBytecode.immutableReferences || {};
