...
contract ENSJobPages is Ownable {
    ...
    modifier onlyJobManagerOrOwner() {
    ...
    function handleHook(uint8 hook, uint256 jobId) external onlyJobManagerOrOwner {
        address manager = jobManager;
        if (manager == address(0)) revert ENSNotConfigured();
        IAGIJobManagerView jobManager = IAGIJobManagerView(manager);
        if (hook == 1) {
