...
contract AGIJobManager is Ownable, ReentrancyGuard, Pausable, ERC721 {
    ...
    function _validateCompletionPayouts(Job storage job) internal view {
    ...
    function _recordCompletion(Job storage job) internal returns (uint256 reputationPoints) {
    ...
    function _agiTypeStats(address nftAddress, uint256 payoutPercentage)
        internal
        view
        returns (uint256 maxPct, bool exists)
    {
    ...
    function _setAgiType(address nftAddress, uint256 payoutPercentage, bool exists) internal {
        if (!exists) {
    ...
    function _completeJob(uint256 _jobId) internal {
        ...
        _validateCompletionPayouts(job);

        job.completed = true;
        job.disputed = false;
        _releaseEscrow(job);

        uint256 reputationPoints = _recordCompletion(job);
    ...
    function _verifyOwnershipAgent(address claimant, string memory subdomain, bytes32[] calldata proof) internal returns (bool) {
        ...
        if (_verifyEnsOwnership(claimant, subdomain, agentRootNode)) {
            emit OwnershipVerified(claimant, subdomain);
            return true;
        }

        if (_verifyEnsOwnership(claimant, subdomain, alphaAgentRootNode)) {
            emit OwnershipVerified(claimant, subdomain);
            return true;
        }
    ...
    function _verifyOwnershipValidator(address claimant, string memory subdomain, bytes32[] calldata proof) internal returns (bool) {
        ...
        if (_verifyEnsOwnership(claimant, subdomain, clubRootNode)) {
            emit OwnershipVerified(claimant, subdomain);
            return true;
        }

        if (_verifyEnsOwnership(claimant, subdomain, alphaClubRootNode)) {
            emit OwnershipVerified(claimant, subdomain);
            return true;
        }
    ...
    function addAGIType(address nftAddress, uint256 payoutPercentage) external onlyOwner whenConfigurable {
        ...
        (uint256 maxPct, bool exists) = _agiTypeStats(nftAddress, payoutPercentage);
        if (maxPct > 100 - validationRewardPercentage) revert InvalidParameters();
        _setAgiType(nftAddress, payoutPercentage, exists);
