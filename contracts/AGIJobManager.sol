...
contract AGIJobManager is Ownable, ReentrancyGuard, Pausable, ERC721 {
    ...
    uint256 internal constant DISPUTE_BOND_BPS = 200;
    ...
    /// @notice Total AGI locked as dispute initiation bonds for unsettled disputes.
    ...
    function _computeAgentBond(uint256 payout, uint256 duration) internal view returns (uint256 bond) {
        ...
        uint256 durationLimit = jobDurationLimit;
        if (durationLimit != 0) {
            unchecked {
                bond = (bond * (durationLimit + duration)) / durationLimit;
            }
        }
    ...
    function disputeJob(uint256 _jobId) external whenNotPaused nonReentrant {
        ...
        if (!job.completionRequested) revert InvalidState();
        uint256 disputeBond = (job.payout * DISPUTE_BOND_BPS) / 10_000;
        if (disputeBond < agentBond) disputeBond = agentBond;
        if (disputeBond > job.payout) disputeBond = job.payout;
        if (disputeBond > 0) {
            _safeERC20TransferFromExact(agiToken, msg.sender, address(this), disputeBond);
            unchecked {
                lockedDisputeBonds += disputeBond;
            }
            job.disputeInitiator = msg.sender;
            job.disputeBondAmount = disputeBond;
        }
        job.disputed = true;
        if (job.disputedAt == 0) {
            job.disputedAt = block.timestamp;
    ...
    function _completeJob(uint256 _jobId, bool repEligible) internal {
        ...
        uint256 reputationPoints = repEligible ? _computeReputationPoints(job) : 0;
    ...
    function _computeReputationPoints(Job storage job) internal view returns (uint256 reputationPoints) {
        uint256 completionTime = job.completionRequestedAt > job.assignedAt
            ...
            : 0;
        unchecked {
            uint256 payoutUnits = job.payout / 1e18;
            uint256 payoutPoints = payoutUnits * payoutUnits * payoutUnits;
            uint256 timeBonus;
