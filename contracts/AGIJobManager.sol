...
contract AGIJobManager is Ownable, ReentrancyGuard, Pausable, ERC721 {
    ...
    uint256 internal constant DISPUTE_BOND_BPS = 100;
    ...
    /// @notice Total AGI locked as dispute initiation bonds.
    ...
    function _bondValidator(uint256 jobId, Job storage job, uint8 vote) internal returns (uint256 bond) {
        bond = job.validatorBondAmount;
        if (bond == 0) {
        ...
        emit ValidatorBonded(jobId, msg.sender, bond, vote);
    }
    ...
    function _computeReputationPoints(Job storage job) internal view returns (uint256 reputationPoints) {
        ...
        unchecked {
            ...
            uint256 base = Math.log2(1 + payoutPoints * 25);
            if (timeBonus > base) {
                timeBonus = base;
            }
    ...
    function _settleDisputeBond(Job storage job, bool agentWins) internal {
        ...
        _t(agentWins ? job.assignedAgent : job.employer, bond);
    }
