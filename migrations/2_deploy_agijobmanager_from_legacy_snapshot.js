...
module.exports = async function (deployer, network, accounts) {
  ...
  const challengePeriod = BigInt(String(rc.challengePeriodAfterApproval || '0'));
  if (challengePeriod > 0n) {
    await maybeSet(manager, 'setChallengePeriodAfterApproval', [challengePeriod.toString()], from);
  } else {
    console.log('Skipping setChallengePeriodAfterApproval because snapshot value is 0 (legacy unavailable/default placeholder).');
  }
