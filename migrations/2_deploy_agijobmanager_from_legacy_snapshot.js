...
const DEFAULT_CHALLENGE_PERIOD_AFTER_APPROVAL = '86400';
...
module.exports = async function (deployer, network, accounts) {
  ...
  } else if (challengePeriodSource !== 'legacy-feature-unavailable') {
    throw new Error(`Snapshot challengePeriodAfterApproval=0 with source=${challengePeriodSource}; refusing silent policy drift.`);
  }
  ...
  if (challengePeriod !== '0') {
    ...
    checks.push([
      'challengePeriodAfterApproval(default)',
      (await manager.challengePeriodAfterApproval()).toString(),
      DEFAULT_CHALLENGE_PERIOD_AFTER_APPROVAL,
    ]);
  }
