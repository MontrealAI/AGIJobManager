...
module.exports = async function (deployer, network, accounts) {
  ...
  const allowZeroChallengePeriodFallback = process.env.ALLOW_CHALLENGE_PERIOD_DEFAULT_ON_ZERO === '1';
  const preReplayChallengePeriod = typeof manager.challengePeriodAfterApproval === 'function'
    ? (await manager.challengePeriodAfterApproval()).toString()
    : null;
  if (challengePeriod === '0') {
    if (!allowZeroChallengePeriodFallback) {
      throw new Error(
        `Snapshot challengePeriodAfterApproval is 0, but setChallengePeriodAfterApproval rejects zero. ` +
        `Refusing silent policy drift. Set ALLOW_CHALLENGE_PERIOD_DEFAULT_ON_ZERO=1 to keep deployed default (${preReplayChallengePeriod}).`
      );
    }
    console.warn(
      `Warning: snapshot challengePeriodAfterApproval=0 cannot be replayed; ` +
      `keeping deployed default (${preReplayChallengePeriod}) due to ALLOW_CHALLENGE_PERIOD_DEFAULT_ON_ZERO=1.`
    );
  } else {
  ...
  if (preReplayChallengePeriod !== null) {
    const expectedChallengePeriod = challengePeriod === '0' ? preReplayChallengePeriod : challengePeriod;
    checks.push(['challengePeriodAfterApproval', (await manager.challengePeriodAfterApproval()).toString(), expectedChallengePeriod]);
  }
