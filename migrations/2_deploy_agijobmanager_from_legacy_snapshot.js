...
module.exports = async function (deployer, network, accounts) {
  ...
  for (const t of snapshot.agiTypes) {
    ...
    const payout = String(t.payoutPercentage || '0');
    const isEnabled = Boolean(t.enabled) && payout !== '0';

    try {
      if (isEnabled) {
        await manager.addAGIType(nft, payout, { from });
      } else {
        await manager.addAGIType(nft, '1', { from });
        await manager.disableAGIType(nft, { from });
      }
    } catch (e) {
      throw new Error(`AGI type replay failed for ${t.nftAddress} enabled=${isEnabled} payout=${payout}: ${e.message}`);
    }
  }
