...
function assertNoUnresolvedLinkReferences() {
  const unresolved = AGIJobManager.unlinked_binary.match(/__\$[a-fA-F0-9]{34}\$__/g) || [];
  if (unresolved.length) {
    throw new Error(`Unresolved AGIJobManager link references detected: ${unresolved.join(', ')}`);
  }
}
...
module.exports = async function (deployer, network, accounts) {
  ...
  assertNoUnresolvedLinkReferences();
