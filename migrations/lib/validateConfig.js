...
function validateConfig(config, web3) {
  ...
  const validatorBondBps = config.parameters.validatorBondBps;
  const validatorBondMin = config.parameters.validatorBondMin;
  const validatorBondMax = config.parameters.validatorBondMax;
  if ([validatorBondBps, validatorBondMin, validatorBondMax].some((x) => x !== null && x !== undefined)) {
    const bps = Number(validatorBondBps ?? 1500);
    const min = Number(validatorBondMin ?? '10000000000000000000');
    const max = Number(validatorBondMax ?? '88888888000000000000000000');
    assert(min <= max, 'parameters.validatorBondMin must be <= parameters.validatorBondMax.');
    if (bps === 0 && min === 0) {
      assert(max === 0, 'parameters.validatorBondMax must be 0 when validatorBondBps=0 and validatorBondMin=0.');
    } else {
      assert(max > 0, 'parameters.validatorBondMax must be > 0 when validator bonds are enabled.');
      assert(!(bps > 0 && min === 0), 'parameters.validatorBondMin must be > 0 when validatorBondBps > 0.');
    }
  }

  const agentBondBps = config.parameters.agentBondBps;
  const agentBondMin = config.parameters.agentBondMin;
  const agentBondMax = config.parameters.agentBondMax;
  if ([agentBondBps, agentBondMin, agentBondMax].some((x) => x !== null && x !== undefined)) {
    const bps = Number(agentBondBps ?? 500);
    const min = Number(agentBondMin ?? '1000000000000000000');
    const max = Number(agentBondMax ?? '88888888000000000000000000');
    assert(min <= max, 'parameters.agentBondMin must be <= parameters.agentBondMax.');
    if (bps === 0 && min === 0 && max === 0) {
      // valid disable mode
    } else {
      assert(max > 0, 'parameters.agentBondMax must be > 0 unless all agent bond params are zero.');
    }
  }

  if (config.parameters.agentBond !== null && config.parameters.agentBond !== undefined) {
    const bond = Number(config.parameters.agentBond);
    const max = Number(config.parameters.agentBondMax ?? '88888888000000000000000000');
    if (max === 0) {
      assert(bond === 0, 'parameters.agentBond must be 0 when agentBondMax is 0.');
    } else {
      assert(bond <= max, 'parameters.agentBond must be <= parameters.agentBondMax.');
    }
  }
