...
async function validateContractAddressCode(label, value, web3) {
  const code = await web3.eth.getCode(value);
  assert(code && code !== '0x', `${label} must be a deployed contract address (non-empty bytecode). Received: ${value}`);
}
...
async function validateConfig(config, web3) {
  ...
  await validateContractAddressCode('identity.agiTokenAddress', config.identity.agiTokenAddress, web3);
  ...
  if (anyNonZeroRoot) {
    ...
    await validateContractAddressCode('identity.ensRegistry', config.identity.ensRegistry, web3);
  }

  if (config.identity.nameWrapper.toLowerCase() !== ZERO_ADDRESS.toLowerCase()) {
    await validateContractAddressCode('identity.nameWrapper', config.identity.nameWrapper, web3);
  }
