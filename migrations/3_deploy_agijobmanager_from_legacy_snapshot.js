...
module.exports = async function (deployer, network, accounts) {
  ...
  if (process.env.ENABLE_LEGACY_SNAPSHOT_MIGRATION !== '1') {
    console.log('Skipping legacy snapshot migration (set ENABLE_LEGACY_SNAPSHOT_MIGRATION=1 to opt in).');
    return;
  }
  ...
  for (const row of SNAPSHOT.agiTypes) {
    const restorePayout = Number(row.restorePayoutPercentage || row.payoutPercentage || '0');
    if (restorePayout > 0) {
      try {
        await manager.addAGIType(row.nftAddress, String(restorePayout), { from: owner });
      } catch (err) {
  ...
  for (const row of SNAPSHOT.agiTypes) {
    if (!row.enabled) {
      try {
        ...
        throw new Error(`disableAGIType failed for ${row.nftAddress}. Snapshot must provide restorePayoutPercentage>0 for disabled entries: ${err.message}`);
      }
    }
  }
