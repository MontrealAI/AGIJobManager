...
module.exports = async function (deployer, network, accounts) {
  ...
  const params = SNAPSHOT.numericParams || {};

  if (params.validationRewardPercentage) {
    await manager.setValidationRewardPercentage(params.validationRewardPercentage, { from: owner });
  }
  if (params.requiredValidatorApprovals) {
    await manager.setRequiredValidatorApprovals(params.requiredValidatorApprovals, { from: owner });
  }
  if (params.requiredValidatorDisapprovals) {
    await manager.setRequiredValidatorDisapprovals(params.requiredValidatorDisapprovals, { from: owner });
  }
  if (params.premiumReputationThreshold) {
    await manager.setPremiumReputationThreshold(params.premiumReputationThreshold, { from: owner });
  }
  if (params.voteQuorum) {
    await manager.setVoteQuorum(params.voteQuorum, { from: owner });
  }
  if (params.maxJobPayout) {
    await manager.setMaxJobPayout(params.maxJobPayout, { from: owner });
  }
  if (params.jobDurationLimit) {
    await manager.setJobDurationLimit(params.jobDurationLimit, { from: owner });
  }
  if (params.completionReviewPeriod) {
    await manager.setCompletionReviewPeriod(params.completionReviewPeriod, { from: owner });
  }
  if (params.disputeReviewPeriod) {
    await manager.setDisputeReviewPeriod(params.disputeReviewPeriod, { from: owner });
  }
  if (params.validatorBondBps && params.validatorBondMin && params.validatorBondMax) {
    await manager.setValidatorBondParams(params.validatorBondBps, params.validatorBondMin, params.validatorBondMax, { from: owner });
  }
  if (params.validatorSlashBps) {
    await manager.setValidatorSlashBps(params.validatorSlashBps, { from: owner });
  }
  if (params.challengePeriodAfterApproval) {
    await manager.setChallengePeriodAfterApproval(params.challengePeriodAfterApproval, { from: owner });
  }
  if (params.agentBondBps && params.agentBondMin && params.agentBondMax) {
    await manager.setAgentBondParams(params.agentBondBps, params.agentBondMin, params.agentBondMax, { from: owner });
  }
  if (params.agentBond) {
    await manager.setAgentBond(params.agentBond, { from: owner });
  }
  ...
  for (const key of numericGetters) {
    if (params[key] !== undefined && typeof manager[key] === 'function') {
      assertEq(key, await manager[key](), params[key]);
    }
  }
