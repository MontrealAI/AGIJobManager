...
module.exports = async function (deployer, network, accounts) {
  ...
  const knownNetworks = new Set(['mainnet', 'development', 'test', 'ganache']);
  if (!knownNetworks.has(network) && process.env.ALLOW_UNKNOWN_NETWORK !== '1') {
    throw new Error(`Unknown network ${network}. Set ALLOW_UNKNOWN_NETWORK=1 to proceed intentionally.`);
  }
