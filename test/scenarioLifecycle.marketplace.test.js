...
contract("AGIJobManager scenario coverage", (accounts) => {
  ...
  it("disputes via disapprovals and resolves agent win with payouts", async () => {
    ...
    assert.ok(
      balancesAfter.validatorA.eq(balancesBefore.validatorA.sub(bond)),
      "incorrect disapproving validators should lose their bond"
    );
    assert.ok(
      balancesAfter.validatorB.eq(balancesBefore.validatorB.sub(bond)),
      "incorrect disapproving validators should lose their bond"
    );
    ...
    const expectedTreasury = payout.sub(expectedAgentPayout).add(bond.muln(2));
    assert.equal(
      balancesAfter.contract.toString(),
      expectedTreasury.toString(),
      "escrow should retain validator rewards plus slashed bonds after dispute resolution"
    );
  });
  ...
  it("preserves escrow on NO_ACTION dispute resolution until later completion", async () => {
    ...
    assert.equal(
      ...
      "escrow should remain locked with validator bond held"
    );
