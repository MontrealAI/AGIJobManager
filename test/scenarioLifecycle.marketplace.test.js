...
contract("AGIJobManager scenario coverage", (accounts) => {
  ...
  it("completes the happy path with payouts, state updates, and NFT issuance", async () => {
    ...
    const applyJobCore = await manager.getJobCore(jobId);
    assert.equal(applyJobCore.assignedAgent, agent, "assigned agent should be set");
    assert.ok(toBN(applyJobCore.assignedAt).gt(toBN(0)), "assignedAt should be recorded");
    ...
    const finalJobCore = await manager.getJobCore(jobId);
    const finalJobValidation = await manager.getJobValidation(jobId);
    assert.strictEqual(finalJobCore.completed, true, "job should be completed");
    assert.strictEqual(finalJobValidation.completionRequested, true, "completionRequested should be true");
    assert.strictEqual(finalJobCore.disputed, false, "disputed should remain false in happy path");
  ...
  it("allows employer to cancel before assignment and refunds escrow", async () => {
    ...
    assert.equal(status.toString(), "0", "job should be deleted");
  ...
  it("completes after a completion request when validators approve", async () => {
    ...
    assert.strictEqual(jobCore.completed, true, "job should complete with validator approvals");
  ...
  it("disputes via disapprovals and resolves agent win with payouts", async () => {
    ...
    const jobCore = await manager.getJobCore(jobId);
    assert.strictEqual(jobCore.completed, true, "job should complete after agent win");
    assert.strictEqual(jobCore.disputed, false, "disputed flag should clear");
  ...
  it("resolves employer-win disputes with refund and no NFT issuance", async () => {
    ...
    assert.ok(disputeTx.logs.find((log) => log.event === "JobDisputed"), "JobDisputed should emit");
    const disputedJobCore = await manager.getJobCore(jobId);
    assert.strictEqual(disputedJobCore.disputed, true, "job should be flagged as disputed");
    await manager.resolveDispute(jobId, "employer win", { from: moderator });

    const jobCore = await manager.getJobCore(jobId);
    assert.strictEqual(jobCore.completed, true, "job should be closed after employer win");
    assert.strictEqual(jobCore.disputed, false, "disputed flag should clear");
  ...
  it("keeps disputes active on NO_ACTION and freezes validator voting", async () => {
    ...
    const midJobCore = await manager.getJobCore(jobId);
    const midJobValidation = await manager.getJobValidation(jobId);
    assert.strictEqual(midJobCore.disputed, true, "disputed flag should remain set");
    assert.strictEqual(midJobCore.completed, false, "job should remain in progress");
  ...
  it("preserves escrow on NO_ACTION dispute resolution until later completion", async () => {
    ...
    const midJobCore = await manager.getJobCore(jobId);
    assert.strictEqual(midJobCore.disputed, true, "disputed flag should remain set");
    assert.strictEqual(midJobCore.completed, false, "job should remain open after neutral resolution");
  ...
  it("pauses state-changing actions and resumes after unpause", async () => {
    ...
    const jobCore = await manager.getJobCore(jobId);
    assert.strictEqual(jobCore.completed, true, "job should complete after unpause");
  });
