...
contract('v0.9.4 posting-time NFT policy', ([owner, employer, agent, validator, wallet30, wallet10, outsider, nextOwner]) => {
  ...
  it('defaults to required and an allowlist exception never bypasses a required NFT', async () => {
  ...
  it('enforces accepted ownership; identity locking does not freeze the operational default', async () => {
    await expectRevert.unspecified(manager.setAgentNftRequired(false, { from: outsider }));
    await manager.transferOwnership(nextOwner);
    await expectRevert.unspecified(manager.setAgentNftRequired(false, { from: nextOwner }));
    await manager.acceptOwnership({ from: nextOwner });
    await expectRevert.unspecified(manager.setAgentNftRequired(false, { from: owner }));
    await manager.lockIdentityConfiguration({ from: nextOwner });
    await manager.setAgentNftRequired(false, { from: nextOwner });
