...
contract("AGIJobManager economic safety", (accounts) => {
  ...
  it("prevents validation reward updates that exceed additional agent payout headroom", async () => {
    const manager = await AGIJobManager.new(
      token.address,
      "ipfs://base",
      ens.address,
    ...
    await expectCustomError(manager.setValidationRewardPercentage.call(15, { from: owner }), "InvalidParameters");
  });

  it("prevents increasing additional agent payout beyond validation headroom", async () => {
    const manager = await AGIJobManager.new(
      token.address,
      "ipfs://base",
      ens.address,
    ...
    await expectCustomError(manager.setAdditionalAgentPayoutPercentage.call(95, { from: owner }), "InvalidParameters");
  });
