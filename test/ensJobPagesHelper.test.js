...
contract("ENSJobPages helper", (accounts) => {
  ...
  it("blocks non-create hooks on unsnapshotted jobs whose preview label collides", async () => {
    const ens = await MockENSRegistry.new({ from: owner });
    const resolver = await MockPublicResolver.new({ from: owner });
    const helper = await ENSJobPages.new(
      ens.address,
    ...
    await expectRevert.unspecified(helper.onCompletionRequested(2, "ipfs://completion2", { from: owner }));
