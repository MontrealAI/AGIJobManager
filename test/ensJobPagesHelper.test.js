...
contract("ENSJobPages helper", (accounts) => {
  ...
  it("rejects prefix changes that would alias a second job onto an existing snapshotted node", async () => {
    const ens = await MockENSRegistry.new({ from: owner });
    const resolver = await MockPublicResolver.new({ from: owner });
    const helper = await ENSJobPages.new(
      ens.address,
    ...
    assert.equal(await helper.jobEnsLabel(12), "agijob12", "original job label remains stable");
    assert.equal(await helper.jobEnsNode(12), sharedNode, "original job node remains stable");
    assert.equal(await resolver.text(sharedNode, "agijobs.spec.public"), "ipfs://spec12", "original node metadata remains intact");
  });
