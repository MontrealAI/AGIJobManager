...
contract('ERC-8004 adapter export (smoke test)', (accounts) => {
  ...
  it('prefers the later typed dispute event when multiple typed events share tx+job', async () => {
    const typedNoAction = {
      event: 'DisputeResolvedWithCode',
      transactionHash: '0xdef',
      blockNumber: 11,
      logIndex: 3,
      returnValues: { jobId: '8', resolutionCode: '0', reason: 'no action' },
    };
    const typedEmployerWin = {
      event: 'DisputeResolvedWithCode',
      transactionHash: '0xdef',
      blockNumber: 11,
      logIndex: 4,
      returnValues: { jobId: '8', resolutionCode: '2', reason: 'employer win' },
    };

    const merged = mergeDisputeResolutionEvents([], [typedNoAction, typedEmployerWin]);
    assert.strictEqual(merged.length, 1, 'duplicate typed settlements should collapse to one event');
    assert.strictEqual(
      merged[0].returnValues.resolutionCode,
      '2',
      'later typed event should win for identical tx+job key'
    );
  });
