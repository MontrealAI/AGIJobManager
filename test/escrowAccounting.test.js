...
contract("AGIJobManager escrow accounting", (accounts) => {
  ...
  it("releases escrow on terminal transitions", async () => {
    ...
    const disputeBondTwo = await computeDisputeBond(manager, payout);
    await token.mint(employer, disputeBondTwo, { from: owner });
    await token.approve(manager.address, disputeBondTwo, { from: employer });
