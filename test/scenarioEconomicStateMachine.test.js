...
contract("AGIJobManager economic state-machine scenarios", (accounts) => {
  ...
  it("resolves disputes with correct economic outcomes", async () => {
    ...
    assert.equal(
      ...
      "escrow should retain validator rewards plus slashed bonds on agent win"
    );
    ...
    assert.ok(
      ...
      "disapproving validators should not regain their bond on agent win"
    );
    assert.ok(
    ...
    assert.equal(
      (await token.balanceOf(manager.address)).toString(),
      expectedTreasury.toString(),
      "escrow should retain validator rewards plus slashed bonds from prior agent win"
    );
  });
});
