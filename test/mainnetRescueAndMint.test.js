...
contract("AGIJobManager rescue hardening", (accounts) => {
  ...
  it("rescueERC20 keeps AGI backing safe and follows withdrawUSDC pause posture", async () => {
    const agi = await MockERC20.new({ from: owner });
    const manager = await deployManager(agi);

    await agi.mint(employer, parseUSDCAmount("10"), { from: owner });
    await agi.approve(manager.address, parseUSDCAmount("10"), { from: employer });
    await manager.createJob("ipfs://spec", parseUSDCAmount("10"), 1000, "details", { from: employer });

    await expectCustomError(
      manager.rescueERC20.call(agi.address, owner, parseUSDCAmount("1"), { from: owner }),
      "InvalidState"
    );
    ...
    await agi.mint(manager.address, parseUSDCAmount("4"), { from: owner });
    await manager.rescueERC20(agi.address, owner, parseUSDCAmount("4"), { from: owner });
    assert.equal((await agi.balanceOf(owner)).toString(), parseUSDCAmount("4"));
  ...
  it("rescueERC20 transfers arbitrary non-USDC tokens", async () => {
    const agi = await MockERC20.new({ from: owner });
    const manager = await deployManager(agi);
