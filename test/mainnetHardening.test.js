...
contract("AGIJobManager mainnet hardening", (accounts) => {
  ...
  it("rescues non-AGI tokens via calldata and blocks AGI token rescue", async () => {
    ...
    await expectCustomError(manager.rescueCall.call("0x0000000000000000000000000000000000000000", erc20Data, { from: owner }), "InvalidParameters");
  });


  it("reverts rescueCall when token returns false or malformed returndata", async () => {
    const agi = await MockERC20.new({ from: owner });
    const ens = await MockENS.new({ from: owner });
    const wrapper = await MockNameWrapper.new({ from: owner });
    const manager = await deployManager(agi, ens.address, wrapper.address);
    ...
    await expectCustomError(manager.rescueCall.call(falseToken.address, transferData, { from: owner }), "TransferFailed");
    await expectCustomError(manager.rescueCall.call(malformedToken.address, transferData, { from: owner }), "TransferFailed");
  });
