...
contract("AGIJobManager mainnet hardening", (accounts) => {
  ...
  it("does not allow malformed ENS tokenURI payloads to brick settlement", async () => {
    ...
    const hugeLen = "f".repeat(62) + "e0";
    const overflowPayload = "0x"
      + "0".repeat(63) + "20"
      + hugeLen
      + "0".repeat(64);
    const managerOverflow = await deployManager(token, ens.address, wrapper.address);
    await managerOverflow.setEnsJobPages(malformed.address, { from: owner });
    await managerOverflow.setUseEnsJobTokenURI(true, { from: owner });
    await prepareSimpleSettlement(managerOverflow, token);
    await malformed.setTokenURIBytes(overflowPayload, { from: owner });
    receipt = await managerOverflow.finalizeJob(0, { from: employer });
    issued = receipt.logs.find((l) => l.event === "NFTIssued");
    assert.equal(await managerOverflow.tokenURI(issued.args.tokenId), "ipfs://base/QmCompletion");
