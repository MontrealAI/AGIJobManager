...
async function fundDisputeBond(token, manager, payer, payout, owner) {
...
contract("AGIJobManager comprehensive", (accounts) => {
  ...
  describe("job lifecycle happy path", () => {
    it("escrows payout, completes, pays agent/validators, and mints NFT", async () => {
      ...
      const agentBond = await computeAgentBond(manager, payout, 2000);
  ...
  describe("dispute resolution behavior", () => {
    ...
    it("resolves agent win and employer win once", async () => {
      ...
      const disputeBondTwo = await fundDisputeBond(token, manager, employer, payout2, owner);
      await manager.disputeJob(jobId2, { from: employer });

      const employerBalanceBefore = new BN(await token.balanceOf(employer));
      await manager.resolveDispute(jobId2, "employer win", { from: moderator });
      const employerBalanceAfter = new BN(await token.balanceOf(employer));
      const agentBond2 = await computeAgentBond(manager, payout2, 1000);
      assert(employerBalanceAfter.sub(employerBalanceBefore).eq(payout2.add(agentBond2).add(disputeBondTwo)));
