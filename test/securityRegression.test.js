...
contract("AGIJobManager security regressions", (accounts) => {
  ...
  it("settles agent-win disputes after completion is requested", async () => {
    ...
    assert.strictEqual(jobCore.completed, true, "agent-win dispute should complete after completion request");
  ...
  it("allows disputes after duration expiry when completion was requested", async () => {
    ...
    const jobCore = await manager.getJobCore(jobId);
    const jobValidation = await manager.getJobValidation(jobId);
    assert.strictEqual(jobCore.completed, true, "agent-win should settle after late completion request");
  ...
  it("allows agent-win dispute resolution while paused after completion request", async () => {
    ...
    const jobCore = await manager.getJobCore(jobId);
    const jobValidation = await manager.getJobValidation(jobId);
    assert.strictEqual(jobCore.completed, true, "agent-win should settle after paused completion request");
  ...
  it("enforces vote rules and dispute thresholds", async () => {
    ...
    const jobCore = await manager.getJobCore(jobIdTwo);
    assert.strictEqual(jobCore.disputed, true, "job should be disputed after threshold");
  });
