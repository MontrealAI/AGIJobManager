...
function run() {
  ...
  const decimalsRaw = args.decimals || '18';
  if (!/^\d+$/.test(String(decimalsRaw))) {
    throw new Error(`Invalid decimals: ${decimalsRaw}. Provide a non-negative integer.`);
  }
  const decimals = Number(decimalsRaw);
  if (!Number.isSafeInteger(decimals)) {
    throw new Error(`Invalid decimals: ${decimalsRaw}. Value is out of safe integer range.`);
  }
