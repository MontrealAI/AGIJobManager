...
const filename = 'ui/agijobmanager_genesis_job_mainnet_2026-03-05-v33.html';
...
check('all three v33 inline scripts parse', () => {
...
check('console manager matches the recorded core deployment', () => {
  const receipt = JSON.parse(fs.readFileSync(path.join(root, 'hardhat/deployments/mainnet/deployment.1.24522684.json')));
  assert.equal(html.match(/const AGI_JOB_MANAGER = "([^"]+)"/)[1].toLowerCase(), receipt.contracts.AGIJobManager.address.toLowerCase());
  assert.equal(html.match(/const KNOWN_ENS_JOB_PAGES_FALLBACK = "([^"]+)"/)[1].toLowerCase(), '0x06188e77c1c38d392b16d9d9fb24673363ce1da0');
});
...
const session = vm.createContext({
  ...
  userAccount:'stale-account', hasAcceptedTerms:true,
