...
meta = root / 'docs/releases/v1.3.0'
...
assert changes and all(p.startswith(('docs/releases/v1.3.0/', 'scripts/release/')) or p == '.github/workflows/current-state-release.yml' for p in changes), 'Release preparation must not change the frozen application.'
out = root / 'build/release/v1.3.0'
expected = {}
...
assert set(expected) == {'AGIJobManager-v1.3.0-COMPLETE.zip', pathlib.Path(config['primaryUI']).name, 'RELEASE_MANIFEST.json', 'SHA256SUMS.txt'}
