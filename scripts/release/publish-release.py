...
for item in config['requiredSourceRuns'].values():
    ...
    print(f'Confirmed historical evidence: {run["name"]} at {source} ({run["html_url"]})')
if not args.publish:
    raise SystemExit(0)
...
assert changes and all(p.startswith(('docs/releases/v0.4.0/', 'scripts/release/')) or p == '.github/workflows/current-state-release.yml' for p in changes), 'Release preparation must not change the frozen application.'
out = root / 'build/release/v0.4.0'
expected = {}
...
assert set(expected) == {'AGIJobManager-v0.4.0-COMPLETE.zip', pathlib.Path(config['primaryUI']).name, 'RELEASE_MANIFEST.json', 'SHA256SUMS.txt'}
