...
meta = root / 'docs/releases/v0.9.3'
...
assert changes and all(p.startswith(('docs/releases/v0.9.3/', 'scripts/release/')) or p == '.github/workflows/current-state-release.yml' for p in changes), 'Release preparation must not change the frozen application.'
out = root / 'build/release/v0.9.3'
expected = {}
...
assert set(expected) == {'AGIJobManager-v0.9.3-COMPLETE.zip', pathlib.Path(config['primaryUI']).name, 'RELEASE_MANIFEST.json', 'SHA256SUMS.txt'}
