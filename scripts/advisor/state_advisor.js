...
function main() {
  ...
  const hasNow = hasValue(data.currentTimestamp);
  const now = hasNow ? toBig(data.currentTimestamp) : null;
  ...
  if (state === 'IN_PROGRESS') {
    const completionStillInWindow = !disputed && (expireAt === null || (now !== null && now <= expireAt));
    if (completionStillInWindow) {
      actions.push('requestJobCompletion (assigned agent)');
    }
    if (expireAt !== null && now !== null && now > expireAt) actions.push('expireJob (available once assignedAt + duration has elapsed)');
  }
  if (state === 'COMPLETION_REQUESTED') {
    if (reviewEndsAt !== null && now !== null && now <= reviewEndsAt) actions.push('validateJob/disapproveJob (eligible validators)');
    if (reviewEndsAt !== null && now !== null && now <= reviewEndsAt) actions.push('disputeJob (employer or assigned agent, if within allowed window)');
    ...
    const reviewElapsed = reviewEndsAt !== null && now !== null && now > reviewEndsAt;
    const challengeElapsed = challengeEndsAt !== null && now !== null && now > challengeEndsAt;
  ...
  if (state === 'DISPUTED') {
    ...
    if (staleDisputeAt !== null && now !== null && now > staleDisputeAt) actions.push('resolveStaleDispute (owner)');
  }

  console.log(`Job ${jobId} advisory (offline)`);
  console.log(`- Derived state: ${state}`);
  console.log(`- now: ${now === null ? '(missing currentTimestamp)' : now}`);
  ...
  if (now === null) {
    console.log('- Warning: missing currentTimestamp; advisor suppresses all now-vs-deadline time-gated actions. Provide currentTimestamp to get actionable timing guidance.');
  }
  ...
  if (state === 'IN_PROGRESS' && !disputed && expireAt !== null && now !== null && now > expireAt) {
