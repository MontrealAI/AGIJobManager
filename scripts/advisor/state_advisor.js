...
function main() {
  ...
  const validatorApproved = data.validatorApproved === undefined ? null : bool(data.validatorApproved);
  ...
  if (state === 'COMPLETION_REQUESTED') {
    ...
    const challengeKnownAndElapsed = challengeEndsAt !== null && now > challengeEndsAt;
    const challengeNotRequired = validatorApproved === false;
    const challengeSatisfied = challengeNotRequired || challengeKnownAndElapsed;
    if (reviewElapsed && challengeSatisfied) {
  ...
  if (state === 'COMPLETION_REQUESTED' && validatorApproved === null && challengeWindow !== null) {
    console.log('- Warning: validatorApproved unknown; finalize advice is suppressed unless challenge status is known safe.');
  }
  ...
  if (state === 'COMPLETION_REQUESTED' && reviewEndsAt !== null) {
    ...
    if (validatorApproved === null && challengeEndsAt === null && challengeWindow !== null) {
      console.log('- Finalize note: provide validatorApproved/validatorApprovedAt to safely model challenge-gated finalization.');
    }
  }
