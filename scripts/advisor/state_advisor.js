...
function main() {
  ...
  const hasCurrentTimestamp = now !== null;
  ...
  if (state === 'IN_PROGRESS' && hasCurrentTimestamp) {
  ...
  if (state === 'COMPLETION_REQUESTED' && hasCurrentTimestamp) {
  ...
  if (state === 'DISPUTED') {
    ...
    if (hasCurrentTimestamp && staleDisputeAt !== null && now > staleDisputeAt) actions.push('resolveStaleDispute (owner)');
  }

  console.log(`Job ${jobId} advisory (offline)`);
  console.log(`- Derived state: ${state}`);
  console.log(`- now: ${hasCurrentTimestamp ? now : 'unknown (provide currentTimestamp)'}`);
  ...
  if (!hasCurrentTimestamp) {
    console.log('- Warning: currentTimestamp is missing; advisor suppresses time-gated actions to avoid revert-prone guidance.');
  }
  ...
  if (hasCurrentTimestamp && state === 'IN_PROGRESS' && !disputed && expireAt !== null && now > expireAt) {
  ...
  if (hasCurrentTimestamp && state === 'COMPLETION_REQUESTED' && reviewEndsAt !== null) {
  ...
  if (hasCurrentTimestamp && state === 'IN_PROGRESS' && expireAt !== null) {
    console.log(`- Earliest expire threshold (must be strictly greater than this): ${expireAt}`);
  }
  if (hasCurrentTimestamp && state === 'DISPUTED' && staleDisputeAt !== null) {
