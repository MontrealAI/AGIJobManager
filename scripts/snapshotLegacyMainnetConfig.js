...
async function main() {
  ...
  if (etherscanKey) {
    ...
    if (process.env.ALLOW_UNSAFE_HTML_FALLBACK !== '1') {
      throw new Error(
        'ETHERSCAN_API_KEY is required for deterministic mutator replay (txlistinternal needed for internal admin calls). ' +
        'To bypass at your own risk, set ALLOW_UNSAFE_HTML_FALLBACK=1.'
      );
    }
    console.warn('Warning: Using unsafe HTML fallback without txlistinternal; snapshot may miss historical internal admin calls.');
