...
async function main() {
  ...
  const agiMap = new Map(); const agiOrder = []; const agiLastPositivePayout = new Map(); let baseIpfsUrl = null;
  ...
  for (const m of mutations) {
    ...
    if (m.functionName==='addAGIType') {
      ...
      if (Number(pct) > 0) agiLastPositivePayout.set(nft, pct);
  ...
  const agiTypes = agiTypesOnchain.map((row) => {
    const restorePayoutPercentage = row.enabled ? row.payoutPercentage : (agiLastPositivePayout.get(row.nftAddress) || null);
    if (!row.enabled && !restorePayoutPercentage) {
      throw new Error(`Cannot build restorable AGI type snapshot for disabled type ${row.nftAddress}: missing historical addAGIType payout in replay history.`);
    }
    return {
      ...row,
      restorePayoutPercentage,
      restorePayoutSource: restorePayoutPercentage ? (row.enabled ? { method: 'eth_call', index: row.source.index } : { method: 'tx_input_replay', nftAddress: row.nftAddress }) : null
    };
  });
  ...
  const snapshot = {
    ...
    agiTypes,
    provenance:{derivedBy:'eth_call + transaction input replay',txCountConsidered:txHashes.length,mutationCount:mutations.length,note:'AGI types are sourced from on-chain agiTypes(index) reads; disabled entries include restorePayoutPercentage derived from replay history to ensure migration can add-then-disable deterministically.'}
  };
