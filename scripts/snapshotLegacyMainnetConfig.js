...
async function main() {
  ...
  const agiTypes = agiTypesOnchain.map((entry) => {
    if (entry.enabled) return entry;
    const replay = agiMap.get(entry.nftAddress);
    if (replay && Number(replay.payoutPercentage) > 0) {
      return { ...entry, restorePayoutPercentage: String(replay.payoutPercentage) };
    }
    return entry;
  });
  ...
  const snapshot = {
    ...
    agiTypes,
    provenance:{derivedBy:'eth_call + transaction input replay',txCountConsidered:txHashes.length,mutationCount:mutations.length,note:'AGI types are sourced from on-chain agiTypes(index) reads; replay data is used for non-enumerable sets and disabled AGI restorePayoutPercentage hints when available.'}
  };
