...
async function main() {
  ...
  const snapshot = {
    ...
    runtimeConfig: {
      ...
      useEnsJobTokenURI: (() => {
        if (typeof viewValues.useEnsJobTokenURI !== 'undefined') return Boolean(viewValues.useEnsJobTokenURI);
    ...
    runtimeConfigProvenance: {
      useEnsJobTokenURI: (() => {
        if (typeof viewValues.useEnsJobTokenURI !== 'undefined') {
          return { source: 'getter', value: Boolean(viewValues.useEnsJobTokenURI) };
        }
        if (dynamic.useEnsJobTokenURI !== null) {
          return {
            source: 'tx-replay',
            value: Boolean(dynamic.useEnsJobTokenURI),
            txCount: dynamic.provenance.useEnsJobTokenURI.length,
            latestTx: dynamic.provenance.useEnsJobTokenURI[dynamic.provenance.useEnsJobTokenURI.length - 1],
          };
        }
        return {
          source: 'tx-replay-inferred-default',
          value: false,
          note: 'No setUseEnsJobTokenURI tx observed in full ordered replay; constructor default is false.',
        };
      })(),
    },
    dynamicSets: {
