...
function getTxHashes(address, apiKey) {
  ...
  for (let page = 1; page <= 500; page += 1) {
    const html = runCurl(`https://etherscan.io/txs?a=${address}&p=${page}`);
    const hits = (html.match(/\/tx\/(0x[0-9a-fA-F]{64})/g) || []).map((x) => x.slice(4).toLowerCase());
    if (hits.length === 0) {
      if (page === 1) {
        throw new Error('Unable to recover tx history from Etherscan HTML fallback.');
      }
      return all;
    }
    for (const h of hits) {
  ...
  throw new Error('Etherscan HTML pagination exceeded safety bound; refusing partial replay.');
}
...
async function main() {
  ...
  const agiMap = new Map(); const agiOrder = []; const agiLastNonZeroPayout = new Map(); let baseIpfsUrl = null;
  ...
  for (const m of mutations) {
    ...
    if (m.functionName==='addAGIType') {
      ...
      if (Number(pct) > 0) agiLastNonZeroPayout.set(nft, pct);
  ...
  const agiTypes = agiTypesOnchain.map((row) => {
    ...
    const restorePayoutPercentage = agiLastNonZeroPayout.get(row.nftAddress);
    if (!restorePayoutPercentage) {
      throw new Error(`Cannot derive restore payout for disabled AGI type ${row.nftAddress}; replay data is insufficient to build a restorable snapshot.`);
    }
    return { ...row, restorePayoutPercentage };
  });
  ...
  const snapshot = {
    ...
    agiTypes,
    provenance:{replayComplete:true,derivedBy:'eth_call + transaction input replay',txCountConsidered:txHashes.length,mutationCount:mutations.length,note:'AGI types are sourced from on-chain agiTypes(index) reads; replay data is used for non-enumerable sets only.'}
  };
