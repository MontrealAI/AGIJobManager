...
async function main() {
  ...
  const hasUseEnsGetter = Object.prototype.hasOwnProperty.call(viewValues, 'useEnsJobTokenURI');
  const supportsUseEnsSetter = abi.some((x) => x.type === 'function' && x.name === 'setUseEnsJobTokenURI');
  let resolvedUseEnsJobTokenURI;
  let useEnsDerivation;
  if (hasUseEnsGetter) {
    resolvedUseEnsJobTokenURI = Boolean(viewValues.useEnsJobTokenURI);
    useEnsDerivation = 'getter';
  } else if (dynamic.useEnsJobTokenURI !== null) {
    resolvedUseEnsJobTokenURI = Boolean(dynamic.useEnsJobTokenURI);
    useEnsDerivation = 'tx-replay';
  } else if (!supportsUseEnsSetter) {
    resolvedUseEnsJobTokenURI = false;
    useEnsDerivation = 'unsupported-in-legacy-abi';
  } else {
    throw new Error(
      'Unable to derive useEnsJobTokenURI: no getter in ABI and no setUseEnsJobTokenURI calls found in tx replay. ' +
      'Provide ETHERSCAN_API_KEY or ensure tx history coverage for deterministic reconstruction.'
    );
  }
  ...
  const snapshot = {
    ...
    runtimeConfig: {
      ...
      useEnsJobTokenURI: resolvedUseEnsJobTokenURI,
      useEnsJobTokenURIDerivation: useEnsDerivation,
      ...
      challengePeriodAfterApproval: viewValues.challengePeriodAfterApproval || '0',
    },
    dynamicSets: {
