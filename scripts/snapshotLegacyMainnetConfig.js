...
function tryDeriveBaseFromTokenURI(rpcUrl, address, blockTag) {
  try {
    ...
    // tokenURI is composed as `${baseIpfsUrl}/${ipfsHash}` in legacy logic.
    // Slice *before* '/Qm' to avoid appending the separator slash into baseIpfsUrl.
...
function getTxsViaHtmlScrape(rpcUrl, address, blockLimitHex) {
  ...
  const maxPages = Number(process.env.ETHERSCAN_HTML_MAX_PAGES || '200');
  let hitMaxPage = true;
  ...
  if (hitMaxPage) {
    throw new Error(`HTML tx scrape reached ETHERSCAN_HTML_MAX_PAGES=${maxPages} without termination; refusing incomplete snapshot. Set ETHERSCAN_HTML_MAX_PAGES higher or provide ETHERSCAN_API_KEY.`);
  }
