...
const OUTPUT_PATH = path.join(__dirname, '..', 'migrations', 'legacy.snapshot.mainnet.0x0178B6baD606aaF908f72135B8eC32Fc1d5bA477.json');
...
async function main() {
  ...
  if (!etherscanKey) {
    throw new Error('ETHERSCAN_API_KEY is required for deterministic extraction in scripts/snapshotLegacyConfig.mainnet.js.');
  }
  ...
  let abi;
  let sourceMeta = { abiSource: 'etherscan-v2-api:getsourcecode' };
  try {
    const fetched = fetchAbiFromEtherscan(LEGACY_ADDRESS, etherscanKey);
    abi = fetched.abi;
    sourceMeta = { abiSource: 'etherscan-v2-api:getsourcecode', ...fetched.sourceMeta };
    sourceMeta.abiSha256 = crypto.createHash('sha256').update(JSON.stringify(abi)).digest('hex');
  } catch (e) {
    throw new Error(`Failed to fetch ABI from Etherscan API: ${e.message}`);
  }
