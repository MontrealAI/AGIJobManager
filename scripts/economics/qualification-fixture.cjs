...
function qualificationFixture({role='agent',now=Math.floor(Date.now()/1000),measurementKind='simulated',reviewPayment='retainer'}={}) {
 ...
 if(reviewPayment==='operator-budget'){model.schemaVersion=1;delete model.reviewRetainers;for(const row of model.cases)delete row.retainerStates;}
 ...
 if(reviewPayment==='operator-budget'){policy.schemaVersion=6;policy.reviewPayment=reviewPayment;}
 ...
 const payload={schemaVersion:policy.schemaVersion,policyDigest:digest(policy),jobId:'0',participantId,jobClass:'bounded-json',issuedAt:now,expiresAt:now+600,specURI:'ipfs://spec',specSha256:digest('spec'),evidence:{reportSha256,measuredAt:now,sampleCount:100,qualification:'qualified',costsIncludeFailuresAndOverhead:true},identities,lifecycle:model,gasBudgetWei:'1000000000000000',ethPriceCeilingUSDC:'3000',commitment};
