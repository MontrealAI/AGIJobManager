...
function parseDisputeResolution(ev) {
...
function dedupeDisputeResolvedEvents(legacyEvents, typedEvents) {
  const byKey = new Map();
  const upsert = (ev, preferTyped) => {
    const jobId = String(ev.returnValues.jobId || ev.returnValues[0] || '');
    const resolution = parseDisputeResolution(ev);
    const key = `${ev.transactionHash || ''}:${jobId}:${resolution}`;
    if (!byKey.has(key) || preferTyped) {
      byKey.set(key, ev);
    }
  };

  for (const ev of legacyEvents) upsert(ev, false);
  for (const ev of typedEvents) upsert(ev, true);
