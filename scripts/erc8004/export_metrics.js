...
function decodeDisputeResolution(ev) {
  const hasTypedResolutionCode = ev.returnValues && ev.returnValues.resolutionCode !== undefined;
  if (hasTypedResolutionCode) {
    const code = Number(ev.returnValues.resolutionCode);
    if (code === 1) return 'agent win';
    if (code === 2) return 'employer win';
    // Typed NO_ACTION/unknown codes are unresolved by definition; do not infer from reason text.
    return '';
  }
