...
function decodeDisputeResolution(ev) {
  const isTyped = (ev.event || '') === 'DisputeResolvedWithCode'
    || (ev.returnValues && ev.returnValues.resolutionCode !== undefined);
  if (isTyped) {
    ...
    // Typed NO_ACTION / unknown codes are unresolved and must not be inferred from reason text.
    return '';
  }
