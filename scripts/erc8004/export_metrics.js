...
function mergeDisputeResolutionEvents(legacyEvents, typedEvents) {
  ...
  for (const ev of legacyEvents.concat(typedEvents)) {
    ...
    const existingTyped = isTypedDisputeResolutionEvent(existing);
    if (incomingTyped && !existingTyped) {
      byKey.set(key, ev);
      continue;
    }
    if (incomingTyped === existingTyped && isEventAfter(ev, existing)) {
      byKey.set(key, ev);
    }
  }
