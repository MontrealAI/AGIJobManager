...
export function actionGates(status: Status) {
  return {
    canApply: status === 'Open',
    canRequestCompletion: status === 'Assigned',
    canDispute: status === 'Completion Requested' || status === 'Assigned',
    canFinalize: status === 'Completion Requested'
  };
}
