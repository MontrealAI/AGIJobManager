...
describe('committed single-file hash navigation', () => {
  ...
  it('keeps normalizeHashHref parseable with a single parsed binding', () => {
    for (const { file, label } of artifactTargets) {
      ...
      const toGatewayMatch = /const toGatewayUrl = \(routeInput\) => \{([\s\S]*?)\n  \};/.exec(html);
      expect(toGatewayMatch, `${label} missing toGatewayUrl helper`).toBeTruthy();
      const toGatewayBody = toGatewayMatch?.[1] ?? '';
      const parsedDeclIdx = toGatewayBody.indexOf('const parsedRoute = parseRouteInput(routeInput);');
      const parsedUseIdx = toGatewayBody.indexOf('parsedRoute.pathname');
      expect(parsedDeclIdx, `${label} toGatewayUrl missing parsedRoute declaration`).toBeGreaterThan(-1);
      expect(parsedUseIdx, `${label} toGatewayUrl missing parsedRoute usage`).toBeGreaterThan(-1);
      expect(parsedDeclIdx, `${label} toGatewayUrl parsedRoute declaration must precede usage`).toBeLessThan(parsedUseIdx);
    }
  });
