...
describe('verify-ipfs script src attribute hardening', () => {
  ...
  it('prefers parseable navigateHashRoute helper when an earlier malformed helper exists', () => {
    const html = `<!doctype html><html><head><meta http-equiv="Content-Security-Policy" content="default-src 'self'; object-src 'none'; frame-ancestors 'none'"><meta name="referrer" content="no-referrer"></head><body>
      <script>const navigateHashRoute = (routePath, mode) => { if (!routePath) return; // malformed on purpose</script>
      <script>
        const navigateHashRoute = (routePath, mode) => {
          if (!routePath || !routePath.startsWith('/')) return;
          if (mode === 'replace') {
    ...
    const body = extractArrowFunctionBodyFromHtml(html, 'navigateHashRoute');
