...
describe('verify-ipfs script src attribute hardening', () => {
  ...
  it('committed artifact keeps rawHash scoped to hashchange listener', () => {
    const committedArtifact = path.resolve(__dirname, '../../agijobmanager.html');
    const html = fs.readFileSync(committedArtifact, 'utf8');
    const navigateRoutePattern = new RegExp('const\\s+navigateHashRoute\\s*=\\s*\\([^)]*\\)\\s*=>\\s*\\{([\\s\\S]*?)\\n\\s*\\};');
    const match = html.match(navigateRoutePattern);

    expect(match).not.toBeNull();
    expect(match?.[1]).not.toMatch(/\brawHash\b/);
  });
