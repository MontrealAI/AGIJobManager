...
function extractArrowFunctionBodyFromHtml(html: string, constName: string): string | null {
  ...
  const scriptCloseTagIndex = html.indexOf('>', scriptOpenIndex);
  if (scriptOpenIndex < 0 || scriptCloseTagIndex < 0 || scriptCloseTagIndex > markerIndex) return null;

  const scriptEndIndex = html.indexOf('</script>', markerIndex);
  if (scriptEndIndex < 0) return null;

  const scriptBody = html.slice(scriptCloseTagIndex + 1, scriptEndIndex);
  return extractArrowFunctionBody(scriptBody, constName);
}
