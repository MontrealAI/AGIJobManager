...
describe('verify-ipfs script src attribute hardening', () => {
  ...
  it('fails when script body injects local script elements', () => {
    const html = `${secureHtml}<script>const s=document.createElement('script');s.src='/_next/static/chunks/app.js';document.head.appendChild(s);</script>`;
    const run = runVerifierWithHtml(html);
    expect(run).toThrow(/Local sidecar fetches detected in script bodies/);
  });
