...
describe('verify-ipfs script src attribute hardening', () => {
  ...
  it('rejects remote embedded assets to preserve single-file guarantees', () => {
    const run = runVerifierWithHtml(`${secureHtml}<img src="https://example.com/logo.svg">`);
    expect(run).toThrow(/Relative or unsupported asset references found/);
  });

  it('allows safe remote navigation links', () => {
    const run = runVerifierWithHtml(`${secureHtml}<a href="https://example.com">external docs</a>`);
    expect(run).not.toThrow();
  });
});
