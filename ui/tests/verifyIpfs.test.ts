...
describe('verify-ipfs script src attribute hardening', () => {
  ...
  it('fails when csp/referrer only appear inside script string literals', () => {
    const html = `<!doctype html><html><head></head><body><script>const a = '<meta http-equiv=Content-Security-Policy>'; const b = "<meta name=referrer>";</script></body></html>`;
    const run = runVerifierWithHtml(html);
    expect(run).toThrow(/CSP meta tag is missing/);
  });

  it('passes with unquoted meta attributes', () => {
    const html = `<!doctype html><html><head><meta http-equiv=Content-Security-Policy content="default-src 'self'"><meta name=referrer content=no-referrer></head><body></body></html>`;
    const run = runVerifierWithHtml(html);
    expect(run).not.toThrow();
  });
});
