...
describe('verify-ipfs script src attribute hardening', () => {
  ...
  it('committed artifact keeps rawHash out of navigateHashRoute helper', () => {
    ...
    const helperMarkerMatches = markerMatches.filter((match) => {
      ...
      const parseableHelper = extractArrowFunctionBody(markerBody, 'navigateHashRoute') !== null;
      const declarationPresent = /\bnavigateHashRoute\s*=\s*\(routePath\s*,\s*mode\)\s*=>\s*\{/.test(markerBody)
        || /\bfunction\s+navigateHashRoute\s*\(routePath\s*,\s*mode\)\s*\{/.test(markerBody);
      return parseableHelper || declarationPresent;
    });
