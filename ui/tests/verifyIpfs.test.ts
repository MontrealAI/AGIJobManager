...
describe('verify-ipfs script src attribute hardening', () => {
  ...
  it('committed root and dist artifacts preserve detectGatewayBase and normalizeHashHref boundaries', () => {
    ...
    const artifacts: ReadonlyArray<readonly [string, string]> = [
      ['root', fs.readFileSync(rootArtifactPath, 'utf8')],
      ['dist', fs.readFileSync(distArtifactPath, 'utf8')]
    ];

    for (const [label, html] of artifacts) {
      const routerScript = extractRouterBootstrapScript(html);
      expect(routerScript, `${label} artifact router bootstrap should exist`).toBeTruthy();

      const detectStart = routerScript?.indexOf('const detectGatewayBase = (pathname) => {') ?? -1;
      const detectEnd = routerScript?.indexOf('const gatewayBase = detectGatewayBase(window.location.pathname);') ?? -1;
      const normalizeStart = routerScript?.indexOf('const normalizeHashHref = (input) => {') ?? -1;
      const clickUse = routerScript?.indexOf('const hashRoute = normalizeHashHref(href);') ?? -1;

      expect(detectStart).toBeGreaterThanOrEqual(0);
      expect(detectEnd).toBeGreaterThan(detectStart);
      expect(normalizeStart).toBeGreaterThan(detectEnd);
      expect(clickUse).toBeGreaterThan(normalizeStart);

      const detectBodyWindow = routerScript?.slice(detectStart, detectEnd) ?? '';
      expect(detectBodyWindow).not.toContain('const href = input;');
      expect(routerScript).toContain('const normalizeHashHref = (input) => {');
    }
  });
