  ...
  const normalizeHashHref = (input) => {
    ...
    if (/^[a-zA-Z][a-zA-Z\d+.-]*:/.test(input) || input.startsWith('//')) {
      try {
        ...
        if (hash.startsWith('#/')) {
          const normalizedPath = stripGatewayBase(parsed.pathname);
          if (!normalizedPath.startsWith('/')) return null;
          if (normalizedPath !== '/' && normalizedPath !== window.location.pathname) return null;
          return hash;
        }

        return toHashRoute(stripGatewayBase(parsed.pathname) + parsed.search);
      } catch {
        return null;
      }
    }
