...
function assertNormalizeHashHrefParsedBinding(singleFileHtml) {
  ...
  if (parsedDeclarations.length !== 1 || !parsedDeclarations[0].startsWith('let parsed')) {
    throw new Error('normalizeHashHref must contain exactly one `let parsed` declaration in generated single-file artifact.');
  }

  if (!/\bparsed\s*=\s*new\s+URL\(input\s*,\s*window\.location\.href\)\s*;/.test(helperBody)) {
    throw new Error('normalizeHashHref must assign parsed from new URL(input, window.location.href).');
  }

  if (/\bconst\s+parsed\b|\bvar\s+parsed\b/.test(helperBody)) {
    throw new Error('normalizeHashHref contains a conflicting parsed declaration and will fail to parse.');
  }
}
