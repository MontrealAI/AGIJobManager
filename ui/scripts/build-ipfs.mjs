...
function locateHeadCloseIndex(documentHtml) {
  const headOpenMatch = documentHtml.match(/<head\b[^>]*>/i);
  if (!headOpenMatch || headOpenMatch.index === undefined) {
    throw new Error('Unable to locate <head> in source HTML.');
  }

  const headOpenEnd = headOpenMatch.index + headOpenMatch[0].length;
  const headCloseIndex = documentHtml.toLowerCase().indexOf('</head>', headOpenEnd);
  if (headCloseIndex < 0) {
    throw new Error('Unable to locate closing </head> in source HTML.');
  }

  return headCloseIndex;
}

const insertBeforeHeadClose = (snippet) => {
  const headCloseIndex = locateHeadCloseIndex(html);
...
{
  const headCloseIndex = locateHeadCloseIndex(html);
  const headSegment = html.slice(0, headCloseIndex);
  const cspRegex = /<meta\b[^>]*http-equiv=["']Content-Security-Policy["'][^>]*>/gi;
  if (cspRegex.test(headSegment)) {
    html = `${headSegment.replace(cspRegex, `  <meta http-equiv=\"Content-Security-Policy\" content=\"${enforcedCsp}\">`)}${html.slice(headCloseIndex)}`;
  } else {
    insertBeforeHeadClose(`  <meta http-equiv=\"Content-Security-Policy\" content=\"${enforcedCsp}\">`);
  }
}

if (!/<meta\b[^>]*name=["']referrer["'][^>]*>/i.test(html.slice(0, locateHeadCloseIndex(html)))) {
