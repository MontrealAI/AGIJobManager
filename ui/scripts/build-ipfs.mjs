...
insertBeforeHeadClose(`<script>(function(){
  ...
  const bootstrapPath = basePath === '/' ? normalized : (basePath + (normalized === '/' ? '' : normalized));

  history.replaceState(history.state, '', bootstrapPath + search);
  window.__IPFS_BOOTSTRAP_ROUTE__ = normalized;

  window.addEventListener('DOMContentLoaded', () => {
    const current = window.location.pathname + window.location.search;
    if (current === (bootstrapPath + search)) {
      const baseUrl = basePath === '/' ? '/' : basePath;
      history.replaceState(history.state, '', baseUrl + search + '#' + normalized);
    }
  }, { once: true });
})();</script>`);
