...
const generatedDeploymentPath = path.join(repoRoot, 'ui/src/generated/deployment.ts');
const generatedDeploymentBefore = fs.existsSync(generatedDeploymentPath) ? fs.readFileSync(generatedDeploymentPath, 'utf8') : '';
execSync('node scripts/sync-deployment.mjs', { cwd: process.cwd(), stdio: 'pipe' });
const generatedDeploymentAfter = fs.readFileSync(generatedDeploymentPath, 'utf8');
if (generatedDeploymentBefore !== generatedDeploymentAfter) {
