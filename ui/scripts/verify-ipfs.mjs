...
function hasMetaWithAttr(sourceHtml, attrName, expectedValue) {
  for (const match of sourceHtml.matchAll(/<meta\b[^>]*>/gi)) {
    const attrs = parseTagAttributes(match[0]);
    const value = (attrs.get(attrName) || '').toLowerCase();
...
if (!hasMetaWithAttr(htmlWithoutScriptBlocks, 'http-equiv', 'content-security-policy')) {
  throw new Error('CSP meta tag is missing from IPFS artifact.');
}

if (!hasMetaWithAttr(htmlWithoutScriptBlocks, 'name', 'referrer')) {
