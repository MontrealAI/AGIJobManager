...
const htmlForAttributeScan = htmlWithoutScripts.replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi, '');
const attributeRefs = [...htmlForAttributeScan.matchAll(/<(a|img|script|link|source|iframe|audio|video|track|embed|object)\b[^>]+\b(src|href|srcset)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'=<>`]+))[^>]*>/gi)]
  .map((m) => ({
    tag: m[1].toLowerCase(),
...
const disallowedFetches = scriptBodies.flatMap((body) => {
  const matches = [...body.matchAll(/\bfetch\(\s*(["'`])(.*?)\1\s*\)/g)].map((m) => m[2].trim());
  return matches.filter((url) => /^(\.\/|\.\.\/|\/)/.test(url));
});
if (disallowedFetches.length > 0) {
  throw new Error(`Relative fetch() references found in scripts: ${disallowedFetches.slice(0, 5).join(', ')}`);
}
