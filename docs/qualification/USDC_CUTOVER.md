...
The checked-in machine report is regenerated for the current repository source and binds the manager source, package versions and ENS deployment tooling. All 23 fork cases pass with preserved legacy inventory and ENS/USDC outcomes. Historical release evidence remains pinned to its release tag; the current namespace tooling postdates the frozen v1.0.0 downloads. See [the v1.0.0 review](OPERATIONS_V097.md) and [current namespace policy](../ENS/DEPLOYMENT_NAMESPACES.md).
...
### ENS resolver API mismatch — corrected in v0.9.2, retained in v1.0.0
...
### ENS namespace reuse — blocked in fresh deployment tooling
...
Fresh deployment tooling derives `usdc-<chainId>-<full-lowercase-manager-address-without-0x>.<parent>` and sets prefix `job-`. It requires an owned parent, an unused root and a manager with no helper or allocated jobs; it rechecks the namespace before deployment. Optional name/node/prefix overrides must match the derived values. Same-manager replacement is an explicit separate mode that preserves the existing root and prefix. See the [namespace policy and three historical helper observations](../ENS/DEPLOYMENT_NAMESPACES.md).

The rehearsal creates the derived wrapped root under `alpha.jobs.agi.eth` through the observed parent owner’s authorization, then operates `job-0` beneath it. ENS Registry reports NameWrapper as its owner; NameWrapper `ownerOf(root)` and `getData(root)[0]` report the new helper as owner of the wrapped token. Token approval is zero and the parent owner has not granted the new helper operator approval. The report's synthetic manager and concrete namespace exist only in the local rehearsal. No new blanket operator approval is granted over the legacy owner's wrapped names. The new root has fuses `0` and inherits expiry `2007731864`; the rehearsal does not remove parent-owner control or establish an immutable namespace. Review that retained authority and expiry before production setup.
...
Validation for v1.0.0 covers **23 cutover scenarios, 8 native-USDC fork scenarios, 94 deployment/preflight/verifier cases, 515 contract/console regressions and 10 actual deployment/size cases**. The Foundry suite contains 37 unit/fuzz/invariant tests. The release validation record binds their final CI results to the exact source commit. Compilation is warning-free; manager runtime is 24,359 bytes, 217 below Ethereum's limit.

The current namespace change reruns the 23 cutover scenarios with the new derived root and expands deployment/preflight/verifier coverage to **98 cases**. Production Solidity and runtime bytecode are unchanged. These current-source results do not replace the frozen release's historical validation record.
...
5. **Wire only the new system.** Review the root derived from the actual manager and chain, recheck availability, then create it with the parent controller's authorization. Confirm the helper's `job-` prefix and both manager/helper pointers. Verify namehash, resolver, wrapper authority and helper owner. Manager ownership requires `acceptOwnership`; ENSJobPages ownership is a separate one-step transfer. Lock configuration only after the reviewed wiring is exercised.
