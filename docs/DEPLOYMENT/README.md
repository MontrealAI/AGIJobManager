...
> v1.0.0 uses immutable six-decimal USDC and retains the v0.9.6 ABI and executable bytecode. A verified v0.9.6 manager remains compatible; first deployments and moves from incompatible older versions require a fresh manager. Read the [USDC migration guide](https://github.com/MontrealAI/AGIJobManager/blob/v1.0.0/docs/USDC_MIGRATION.md) before following operational examples. Historical receipts are not USDC deployments.
...
> These guides preserve historical references; their old deployment commands are retired. Use the Hardhat guide above for v1.0.0 and later.
