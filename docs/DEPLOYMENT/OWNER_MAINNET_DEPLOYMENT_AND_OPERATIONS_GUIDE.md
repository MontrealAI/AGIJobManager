...
### 6.3 Required config edit before any migration

Open `migrations/config/agijobmanager.config.js` and populate approved values before running dry-run or mainnet migration.

Minimum required fields for mainnet safety:
- `ownership.finalOwner` (must not be `null` on mainnet),
- `identity.agiTokenAddress` (default is `$AGIALPHA`, change only if formally approved),
- `authorizationRoots` / `rootNodes` and/or `merkleRoots` according to your allowlist policy,
- `protocolParameters` values that differ from defaults,
- `dynamicLists` (moderators/additional lists/blacklists) if you intend to preload them.

If `ownership.requireFinalOwnerOnMainnet` remains `true` (default), migrations will intentionally fail on Ethereum Mainnet until `ownership.finalOwner` is set to a valid address.
