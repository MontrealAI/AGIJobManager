...
- Success check: confirm the `AGITypeUpdated` event for your `nftAddress`, then read `agiTypes(0)`, `agiTypes(1)`, and so on until the first out-of-bounds revert (this indicates you passed `agiTypes.length`). Within the successful rows, match the one whose `nftAddress` equals your target address and confirm its payout percentage equals the intended value.
