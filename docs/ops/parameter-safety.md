...
5. **Verify recovery:**
   - Query `getJobCore(jobId)`, `getJobValidation(jobId)`, and `getJobStatus(jobId)` to confirm `completed` and `disputed` flags.
...
| `InvalidState` | Job actions, disputes, listings | Job not in expected lifecycle state (e.g., already completed, already assigned, listing inactive). | Read `getJobCore(jobId)` / `getJobValidation(jobId)` to confirm state; retry appropriate action. |
