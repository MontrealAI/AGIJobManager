...
| `clubRootNode`, `agentRootNode`, `alphaClubRootNode`, `alphaAgentRootNode` (`setRootNodes`) | ENS namehash | Eligibility gating. | Must match intended ENS hierarchy; update only before any job exists and before `lockConfiguration()`. | Wrong root nodes → `_verifyOwnership` fails → validators/agents cannot qualify. | Update before lock or use `additional*` allowlist; redeploy if pervasive. |
| `validatorMerkleRoot`, `agentMerkleRoot` (`set*MerkleRoot`) | Merkle root | Eligibility gating. | Must match allowlists; updateable post‑lock (allowlist‑only). | Bad root means Merkle proofs always fail; gating relies solely on ENS or allowlist. | Update Merkle roots or use `additional*` allowlist. |
| `ens`, `nameWrapper` (`setEnsConfig`) | contract address | ENS/NameWrapper ownership checks. | Must be correct chain-specific addresses; update only before any job exists and before `lockConfiguration()`. | Wrong addresses → ownership checks fail; `_verifyOwnership` emits recovery events and returns false. | Update before lock or use `additional*` allowlist; redeploy if pervasive. |
