...
5. **Token transfer failures (blacklist / frozen ERC‑20)**
   ...
   - **Failure:** settlement reverts during payouts.
...
| `NotAuthorized` | Role-gated paths | Caller lacks role/ownership (agent not assigned, validator not allowlisted). | Verify role gating, allowlists, and ownership; add via owner if needed. |
