...
| `additionalAgentPayoutPercentage` (`setAdditionalAgentPayoutPercentage`) | uint256 percentage | Legacy parameter retained for compatibility; does **not** affect payout eligibility or settlement. | `1..100` on-chain; `additionalAgentPayoutPercentage + validationRewardPercentage <= 100` enforced on set. | No direct settlement impact; still subject to on-chain bounds. | Keep low to avoid confusion; adjust if needed for legacy tooling. |
...
| `addAdditionalAgent` / `addAdditionalValidator` | allowlist | Eligibility bypass (identity checks only). | Only use for emergency recovery or vetted identities. | Overuse weakens gating; underuse when Merkle/ENS config is wrong can stall jobs. Additional agents must still have a nonzero AGIType payout tier. | Add temporary allowlist entries; remove later. |
