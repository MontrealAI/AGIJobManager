...
| --- | --- | --- | --- | --- | --- |
| `agiToken` (fixed) | ERC‑20 address | Escrow deposits, payouts, marketplace purchases, reward pool, `withdrawAGI`. | Enforced to the canonical AGI token `0xA61a…` at deployment. | Deployment reverts if a non-canonical token is provided. | Redeploy with the correct token address. |
...
| `clubRootNode`, `agentRootNode` (constructor) | ENS namehash | Eligibility gating. | Enforced to canonical roots (`club.agi.eth` and `agent.agi.eth`) with alpha.* derived on-chain. | Deployment reverts if non-canonical roots are provided. | Redeploy with canonical roots. |
