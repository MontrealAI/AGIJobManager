...
- **Validator payout**: on completion, only **correct** validators split `job.payout * validationRewardPercentage / 100` plus any pooled bond amounts. Incorrect validators receive their bond minus the slashed amount. If no validators, the validator budget is returned to the employer.
- **Refunds**:
