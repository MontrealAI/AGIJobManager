...
| Agent NFT requirement | `setAgentNftRequired(bool)` changes the default for future jobs only; disabled on fresh v1.0.3 deployments. Each posted job keeps its recorded policy. |
...
1. Open the v1.0.3 USDC console, select the verified manager and connect as its owner. Confirm the network, manager and current wallet addresses.
