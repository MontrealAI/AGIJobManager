...
1. `address _agiTokenAddress` — **must** be the canonical AGI token (`0xA61a…`).
...
5. `bytes32 _clubRootNode` — **must** equal `namehash("club.agi.eth")`.
6. `bytes32 _agentRootNode` — **must** equal `namehash("agent.agi.eth")`.
...
### Escrow token invariant
- `agiToken` is fixed to the canonical AGI token and cannot be changed post-deploy.
- **Mainnet invariant**: production deployments must keep `agiToken` fixed to `0xA61a3B3a130a9c20768EEBF97E21515A6046a1fA`.
