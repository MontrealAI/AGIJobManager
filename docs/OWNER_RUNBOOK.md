...
1. Check out the immutable v0.9.6 release and verify its checksums. Use Node 22.23.2 and the committed root and Hardhat lockfiles.
...
USDC is immutable and the 30% / 10% shares are fixed in v0.9.6. Recipient addresses can change only with paused intake and zero reserves; existing escrow cannot be redirected to a new wallet.
