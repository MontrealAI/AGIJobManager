...
1. Check out the immutable v1.0.2 release and verify its checksums. Use Node 22.23.2 and the committed root and Hardhat lockfiles.
...
USDC is immutable and the 30% / 10% shares are fixed in v1.0.2. Recipient addresses can change only with paused intake and zero live job escrow and bonds. Previously reserved payment claims keep their original beneficiaries; wallet rotation does not redirect them.
