...
| `ens` | `updateEnsRegistry` | Yes | identity-configurable + nonzero + empty locked balances (`_requireEmptyEscrow`) | Identity gating dependency |
| `nameWrapper` | `updateNameWrapper` | Yes | identity-configurable + nonzero + empty locked balances (`_requireEmptyEscrow`) | Wrapped-root checks dependency |
...
| Root nodes | `updateRootNodes` | Yes | identity-configurable + empty locked balances (`_requireEmptyEscrow`) | club/agent + alpha variants |
