...
- **Root governance**: ENS root nodes are changeable only before any job exists and before `lockConfiguration()`. Merkle roots remain updateable post‑lock and are access-only allowlists.
