# AGIJobManager security considerations — v0.9.7
...
- Incoming funds move exactly and atomically. A failed outgoing transfer becomes a protected claim backed by lockedClaims, without blocking other eligible recipients. Retry claimUSDC to the original beneficiary once the restriction is resolved.
...
The owner retains power over pauses, eligibility, moderators and stale disputes. Moderators decide disputes. Validators must assess real evidence; bonds do not prove honest judgment. No-vote finalization opens a dispute and does not pay the agent. A neutral timeout returns original funds if arbitration remains unanswered. ENS and metadata systems can fail or provide misleading information. Circle retains issuer powers over USDC.
