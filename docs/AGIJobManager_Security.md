...
- **Phantom job IDs / takeover**: `_job` reverts when the job’s employer is the zero address, preventing interaction with deleted or nonexistent job records through the getters and lifecycle actions that rely on `_job`.
