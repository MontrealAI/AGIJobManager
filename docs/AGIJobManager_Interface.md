...
Selected state variables and their accessors.
...
- `getJobCore(jobId)`: returns core job fields (assignment, payout, completion/dispute flags).
- `getJobValidation(jobId)`: returns validation/dispute counters and timestamps.
- `getJobMetadata(jobId)`: returns job specification and completion URIs plus details.
...
| `getJobCore(jobId)` | tuple | Core job fields (no mappings/arrays). | — |
| `getJobValidation(jobId)` | tuple | Validation counters + timestamps. | — |
| `getJobMetadata(jobId)` | tuple | Job spec/completion metadata. | — |
