...
- `agentNftRequired()` starts `false` on fresh managers and is the default for future postings; `setAgentNftRequired(bool)` is owner-only; `jobAgentNftRequired(jobId)` reads the immutable posting-time choice. NFT collection mutations require zero live job escrow and bonds. See [NFT policy](NFT_POLICY.md).
