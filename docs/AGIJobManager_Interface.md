...
- `AGI_TOKEN_ADDRESS`: fixed ERC‑20 address used for escrow and payouts.
- `agiToken`: ERC‑20 token used for escrow and payouts (constant address).
...
- `clubRootNode`, `clubRootNodeAlpha`, `agentRootNode`, `agentRootNodeAlpha`: ENS root nodes for validators and agents (base + alpha).
...
| --- | --- | --- | --- |
| `AGI_TOKEN_ADDRESS()` | `address` | Fixed ERC‑20 token address. | — |
...
| `agentRootNodeAlpha()` | `bytes32` | ENS alpha root node for agents. | — |
...
| `clubRootNodeAlpha()` | `bytes32` | ENS alpha root node for validators. | — |
