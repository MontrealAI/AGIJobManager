...
> Note: `RootNodeUpdated` and `MerkleRootUpdated` are emitted by `updateRootNodes` and `updateMerkleRoots`, respectively.
...
| `updateAGITokenAddress(newToken)` | Owner | — | Changes ERC‑20 used for escrow/payouts (pre‑job + pre‑lock only). | `InvalidParameters`, `InvalidState`, `ConfigLocked`. |
| `updateENSRegistry(newEns)` | Owner | — | Updates ENS registry address (pre‑job + pre‑lock only). | `InvalidParameters`, `InvalidState`, `ConfigLocked`. |
| `updateNameWrapper(newNameWrapper)` | Owner | — | Updates NameWrapper address (pre‑job + pre‑lock only). | `InvalidParameters`, `InvalidState`, `ConfigLocked`. |
| `updateRootNodes(club,agent,alphaClub,alphaAgent)` | Owner | — | Updates ENS root nodes (pre‑job + pre‑lock only). | `InvalidState`, `ConfigLocked`. |
| `updateMerkleRoots(validatorRoot,agentRoot)` | Owner | — | Updates Merkle allowlist roots (access control only). | Standard `Ownable` check. |
