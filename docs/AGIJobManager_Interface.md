...
- `getJobValidation(jobId)`: returns completion/validator/dispute metadata.
- `getJobUris(jobId)`: returns job spec/completion URI + stored metadata.
...
| `getJobValidation(jobId)` | tuple | Validation/dispute flags and timestamps. | — |
| `getJobUris(jobId)` | tuple | Job spec + completion URIs and metadata strings. | — |
