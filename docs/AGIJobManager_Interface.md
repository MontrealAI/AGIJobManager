...
- `AGI_TOKEN_ADDRESS`: fixed AGI token address (`0xA61a…`), returned by `agiToken()`.
- `CLUB_ROOT_NODE`, `AGENT_ROOT_NODE`: fixed ENS root nodes (envless) for validators and agents.
- `clubRootNode`, `agentRootNode`: active ENS root nodes (fixed to the canonical values).
