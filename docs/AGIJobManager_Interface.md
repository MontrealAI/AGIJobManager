...
- `getJobValidation(jobId)`: returns validator counters and dispute/completion timestamps.
- `getJobMetadata(jobId)`: returns job spec/completion URIs and details.
...
| `getJobValidation(jobId)` | tuple | Validator counters and completion/dispute timestamps. | — |
| `getJobMetadata(jobId)` | tuple | Job spec/completion URIs and details. | — |
