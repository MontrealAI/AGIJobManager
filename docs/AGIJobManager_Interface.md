...
- `getJobCore(jobId)`: returns core job fields (no mappings/arrays).
- `getJobValidation(jobId)`: returns validation/completion counters and timestamps.
- `getJobUris(jobId)`: returns job spec and completion URIs.
- `getJobDetails(jobId)`: returns the job details string.
...
| `getJobCore(jobId)` | `(address,address,uint256,uint256,uint256,bool,bool,bool,uint8)` | Core job fields (no mappings/arrays). | `JobNotFound`. |
| `getJobValidation(jobId)` | `(bool,uint256,uint256,uint256,uint256)` | Completion flags, validator counts, timestamps. | `JobNotFound`. |
| `getJobUris(jobId)` | `(string,string)` | Job spec URI and completion URI. | `JobNotFound`. |
| `getJobDetails(jobId)` | `string` | Job details string. | `JobNotFound`. |
