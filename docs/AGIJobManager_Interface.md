...
- `getJobValidatorCount(jobId)` / `getJobValidatorAt(jobId,index)`: returns validator set size and indexed validator addresses.
...
| `getJobValidatorCount(jobId)` | `uint256` | Validator count for a job. | `JobNotFound`. |
| `getJobValidatorAt(jobId,index)` | `address` | Validator address at index. | `JobNotFound`, `InvalidParameters`. |
