...
- `getJobURIs(jobId)`: returns job spec/completion metadata URIs and details text.
- Validator participation is tracked via events (`JobValidated`/`JobDisapproved`) for off‑chain indexing.
...
| `updateTermsAndContact(hash,email)` | Owner | — | Updates metadata fields together. | Standard `Ownable` check. |
| `updateAdditionalTexts(text1,text2,text3)` | Owner | — | Updates the informational metadata fields together. | Standard `Ownable` check. |
...
| `getJobURIs(jobId)` | `(string,string,string,string)` | Job spec URI, completion URI, IPFS hash, details. | `JobNotFound`. |
