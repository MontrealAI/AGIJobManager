...
- `getJobMeta(jobId)`: returns validation counters, timestamps, and job URIs/details.
...
| `getJobMeta(jobId)` | `(bool,uint256,uint256,uint256,uint256,string,string,string)` | Completion flags, validator counts, timestamps, URIs, and details. | `JobNotFound`. |
