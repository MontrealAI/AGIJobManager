...
- `getJobState(jobId)`: returns core job fields.
- `getJobValidation(jobId)`: returns validation counters/timestamps.
- `getJobMetadata(jobId)`: returns job spec URI, completion URI, and details string.
...
| `getJobState(jobId)` | `(address,address,uint256,uint256,uint256,bool,bool,bool,uint8)` | Core job fields. | `JobNotFound`. |
| `getJobValidation(jobId)` | `(bool,uint256,uint256,uint256,uint256)` | Validation counters/timestamps. | `JobNotFound`. |
| `getJobMetadata(jobId)` | `(string,string,string)` | Job spec URI, completion URI, details. | `JobNotFound`. |
