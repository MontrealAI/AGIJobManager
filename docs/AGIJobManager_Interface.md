...
Most are public and have autogenerated getters; job storage is internal with explicit read accessors.
...
- `getJobCore(jobId)`: core job fields (employer, assignment, payout, flags).
- `getJobValidation(jobId)`: validation flags + counters/timestamps.
- `getJobURIs(jobId)`: job spec/completion URIs and metadata strings.
- `getJobValidatorCount(jobId)` / `getJobValidatorAt(jobId,index)`: validator list access.
...
| `getJobCore(jobId)` | `(address,address,uint256,uint256,uint256,bool,bool,bool,uint8)` | Core job fields (employer, assignment, payout, flags). | `JobNotFound`. |
| `getJobValidation(jobId)` | `(bool,uint256,uint256,uint256,uint256)` | Validation flags + counters/timestamps. | `JobNotFound`. |
| `getJobURIs(jobId)` | `(string,string,string,string)` | Job spec/completion URIs and metadata strings. | `JobNotFound`. |
| `getJobValidatorCount(jobId)` | `uint256` | Validator count for the job. | `JobNotFound`. |
| `getJobValidatorAt(jobId,index)` | `address` | Validator address by index. | `JobNotFound`, `InvalidParameters`. |
