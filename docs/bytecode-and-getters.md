# bytecode and getters — v0.9.5

The development source adds `getJobBonds(jobId)` after v0.9.5. It returns current outstanding `agentAmount`, per-reviewer `validatorAmount`, `validatorFixed`, and `disputeAmount`. A fixed zero bond must not be mistaken for an unset bond. Settlement clears these fields; use transaction history for past deposits. The new getter changes runtime bytecode and is absent from existing managers. See [compatibility and verification](qualification/BUYER_ECONOMICS_FOLLOWUP.md).
...
Previous detailed instructions that describe NFT-based payout shares or retained job-cost revenue are historical and remain in [v0.5.0](https://github.com/MontrealAI/AGIJobManager/tree/v0.5.0). Do not use them to configure v0.9.5.
