# bytecode and getters — v0.9.6

v0.9.6 adds `getJobBonds(jobId)`. It returns current outstanding `agentAmount`, per-reviewer `validatorAmount`, `validatorFixed`, and `disputeAmount`. A fixed zero bond must not be mistaken for an unset bond. Settlement clears these fields; use transaction history for past deposits. The getter is absent from v0.9.5 and older managers and requires a fresh deployment. See [compatibility and verification](qualification/BUYER_ECONOMICS_FOLLOWUP.md).
...
Previous detailed instructions that describe NFT-based payout shares or retained job-cost revenue are historical and remain in [v0.5.0](https://github.com/MontrealAI/AGIJobManager/tree/v0.5.0). Do not use them to configure v0.9.6.
