...
| --- | --- | --- | --- |
| Contract suite | Full compile + Truffle tests + bytecode guard | `npm test` | Core protocol behavior; uses disposable local accounts |
...
| Foundry gate | Fuzz and stateful invariants | `FOUNDRY_PROFILE=ci forge test` | Payout/reserve conservation, owner-change snapshots and terminal settlement |
...
Legacy Truffle/Ganache dependencies retain documented development advisories. Never use production keys or untrusted inputs with these local test tools. See [dependency scope](DEPENDENCY_SECURITY.md). A fork test proves behavior at its pinned block; deployment preflight must recheck current USDC and instance state.
