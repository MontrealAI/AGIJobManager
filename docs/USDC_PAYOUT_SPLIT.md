...
5. Close existing jobs on their original contracts and under their original economics. v0.7.0 cannot upgrade earlier immutable contracts, migrate escrow, or reuse old approvals.

The interfaces require the v0.7.0 wallet and pending-owner getters and canonical USDC before enabling writes, display both recipient addresses, and use new browser storage namespaces. Token/network/getter checks complement source and bytecode verification; they do not authenticate arbitrary contracts supplied by a user.

## Compatibility

- v0.7.0 retains the v0.6.0 payout flow, wallet getters and `JobPayoutDistributed`. It adds guarded `setSettlementWallets`, `pendingOwner`, `acceptOwnership` and `OwnershipTransferStarted`; `transferOwnership` now proposes a handover. Renunciation is disabled. `PlatformRevenueAccrued` remains retired.
