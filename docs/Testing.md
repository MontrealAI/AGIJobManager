...
By default the Truffle config keeps `viaIR` disabled; stack-too-deep is avoided by removing the giant autogenerated `jobs` getter from the ABI.
Keep compiler settings consistent for verification.
