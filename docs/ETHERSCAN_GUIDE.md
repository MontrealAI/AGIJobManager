...
- `getJobValidation`: include `completionRequested`, `completionRequestedAt`, `disputedAt`, and if available `validatorApproved` + `validatorApprovedAt` for challenge-window accurate finalize timing.
- also provide current timestamp and protocol windows when needed (`completionReviewPeriod`, `disputeReviewPeriod`, `challengePeriodAfterApproval`).
