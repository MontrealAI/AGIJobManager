...
Advisor input should include `assignedAt` from `getJobCore` and `completionRequested`, `completionRequestedAt`, `disputedAt`, `completionReviewPeriod`, `disputeReviewPeriod` from `getJobValidation` for accurate timing outputs.
