...
- `viaIR = true` (required to avoid stack-too-deep errors with the current contract size).
...
The deployment script in `migrations/2_deploy_contracts.js` reads constructor parameters from environment variables and applies mainnet defaults when `network=mainnet`/chainId=1. Override addresses/roots via `.env` for Sepolia/local/private deployments.
