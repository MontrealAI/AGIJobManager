...
| `SOLC_VERSION` / `SOLC_RUNS` / `SOLC_VIA_IR` / `SOLC_EVM_VERSION` | Compiler settings | Defaults: `SOLC_VERSION=0.8.28`, `SOLC_RUNS=200`, `SOLC_VIA_IR=false`, `SOLC_EVM_VERSION=london`. |
...
> **Compiler note**: `AGIJobManager.sol` uses `pragma solidity ^0.8.28`, while the default Truffle compiler is `0.8.28`. For reproducible verification, keep `SOLC_VERSION`, optimizer runs, and `viaIR` consistent with the original deployment.
...
- `viaIR = false` by default (enable only if stack-too-deep errors require it).
