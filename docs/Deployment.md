...
> **Compiler note**: `AGIJobManager.sol` uses `pragma solidity ^0.8.33`, while the default Truffle compiler is `0.8.33`. For reproducible verification, keep `SOLC_VERSION`, optimizer runs, and the `viaIR` flag consistent with the original deployment. `viaIR` is enabled by default because the runtime bytecode must remain under the EIP‑170 limit.
...
- `viaIR = true` by default (required to keep runtime bytecode under the EIP‑170 limit).
...
If you deploy with `SOLC_VIA_IR=true` or the plugin cannot verify, use Etherscan’s **Solidity (Standard-Json-Input)** flow:
