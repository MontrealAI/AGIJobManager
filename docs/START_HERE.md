# Start here — AGIJobManager v1.4.0
...
Read [what this release includes and how existing managers remain compatible](V1_RELEASE_SCOPE.md). [v1.4.0 adds lifecycle economics and signed qualification checks](V1_RELEASE_SCOPE.md#published-download-versus-current-source).
...
Download the [v1.4.0 complete package](https://github.com/MontrealAI/AGIJobManager/releases/download/v1.4.0/AGIJobManager-v1.4.0-COMPLETE.zip) or [standalone USDC console](https://github.com/MontrealAI/AGIJobManager/releases/download/v1.4.0/agijobmanager-usdc.html) from the repository's release page. Check its `SHA256SUMS.txt` before use. Open `agijobmanager-usdc.html` in a browser with an Ethereum wallet. It needs internet access for the integrity-pinned Web3 library, wallet/RPC communication and display resources; it is not an offline transaction application. Never enter a seed phrase or private key into the console. Public submissions must contain no added personal information, confidential material or secrets; read the [user-data rules](LEGAL/USER_DATA_RULES.md). Draft saving is explicit and unencrypted. Review each public-content confirmation before an upload or submission.
