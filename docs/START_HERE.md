...
Read [what 1.0 includes and how existing managers remain compatible](V1_RELEASE_SCOPE.md). [v1.0.1 includes the current console improvements and deployment-specific ENS names](V1_RELEASE_SCOPE.md#published-download-versus-current-source).
...
Download the [v1.0.1 complete package](https://github.com/MontrealAI/AGIJobManager/releases/download/v1.0.1/AGIJobManager-v1.0.1-COMPLETE.zip) or [standalone USDC console](https://github.com/MontrealAI/AGIJobManager/releases/download/v1.0.1/agijobmanager-usdc.html) from the repository's release page. Check its `SHA256SUMS.txt` before use. Open `agijobmanager-usdc.html` in a browser with an Ethereum wallet. It needs internet access for the integrity-pinned Web3 library, wallet/RPC communication and display resources; it is not an offline transaction application. Never enter a seed phrase or private key into the console.
