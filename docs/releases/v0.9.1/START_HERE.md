...
4. **Operators:** follow `source/hardhat/README.md` and `source/docs/MAINNET_READINESS.md`. Use Node 22.23.2, committed lockfiles and the exact qualified compiler configuration. Install both root and Hardhat workspaces. Truffle deployment commands are retired.
5. **Existing deployments:** v0.9.1 changes production code, so a fresh deployment is required for these fixes. Existing jobs, balances and approvals stay on the original manager. Reconcile and settle there; do not assume publication migrates anything.
