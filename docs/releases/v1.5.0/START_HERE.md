# AGIJobManager v1.5.0 — start here
...
3. **Prepare measured inputs.** Read `source/docs/OPERATIONS/QUALIFIED_ADMISSION.md`. It defines schema v2 exact vote/delivery commitments, fixed reviewer ballots, preparation-attempt limits, canonical observations and durable-reservation responsibilities. New action-sensitive runners should reject legacy v1 packets. Use `npm run economics:admission -- --help`. A passing calculation or signature is not transaction authority.
...
Private Agent, Node and Fleet applications are excluded. The annotated tag points to qualified application source; this archive's top-level evidence and `release-tooling` describe the v1.5.0 publication. Read `RELEASE_NOTES.md` and `VALIDATION.md`.
