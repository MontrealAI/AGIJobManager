...
| `additionalAgentPayoutPercentage` | Owner (`setAdditionalAgentPayoutPercentage`) | Legacy parameter retained for compatibility; does **not** affect payout eligibility or settlement. | `1..100`; `additionalAgentPayoutPercentage + validationRewardPercentage <= 100` enforced on set. | Keep low to avoid confusing operator expectations (no payout impact). | No direct settlement impact; still subject to on-chain bounds. |
| `additionalValidators`, `additionalAgents` | Owner (`addAdditionalValidator`, `addAdditionalAgent`) | Manual allowlist bypass for eligibility checks. | None. | Use as a recovery tool when allowlist/ENS config blocks participation. | Overuse weakens trust; underuse when root nodes are wrong can stall jobs. Additional agents must still have a nonzero AGIType payout tier. |
