...
| `ens`, `nameWrapper` | Owner (`updateENSRegistry`, `updateNameWrapper`) | ENS/NameWrapper ownership checks for validators/agents. | Must be set **before any job exists** and **before `lockConfiguration()`**. | Set chain‑specific addresses and lock after verification. | Mis-set addresses cause ownership checks to fail; use `additional*` allowlist to recover before the lock or redeploy. |
| `clubRootNode`, `agentRootNode` | Owner (`updateRootNodes`) | Gate eligibility for validators/agents via ENS ownership. | Can be set **before any job exists** and **before `lockConfiguration()`**. | Validate before deployment and lock after confirmation. | Mis-set roots can prevent validators/agents from qualifying; use `additional*` allowlist to recover pre‑lock or redeploy. |
| `validatorMerkleRoot`, `agentMerkleRoot` | Owner (`setValidatorMerkleRoot`, `setAgentMerkleRoot`) | Gate eligibility for validators/agents via Merkle allowlists. | None. | Keep allowlists updated; Merkle roots **do not** control payouts. | Mis-set roots cause Merkle proofs to fail; recovery by updating roots or using `additional*` allowlists. |
...
- **Root cause:** Thresholds exceed the available validator set, or Merkle/ENS gating blocks validators from proving eligibility. Roots must be corrected before lock (or allowlists used).
- **On‑chain recovery:**
