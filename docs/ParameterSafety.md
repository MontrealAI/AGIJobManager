...
| --- | --- | --- | --- | --- | --- |
| `agiToken` | **Immutable (fixed)** | ERC‑20 used for escrow deposits, payouts, reward pool, and withdrawals. | Enforced to `0xA61a…` in constructor. | No change permitted. | Deployment reverts if the token address is not the canonical AGI token. |
...
- Token address is fixed to the canonical AGI token; no post-deploy changes are possible.
