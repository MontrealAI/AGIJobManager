...
| --- | --- | --- | --- | --- | --- |
| `agiToken` | Constructor (immutable) | ERC‑20 used for escrow deposits, payouts, reward pool, and withdrawals. | None. | **Immutable after deploy.** Confirm correctness before launch. | If deployed with the wrong token, escrow and payout flows will be misconfigured; redeploy is required to correct it. |
...
- **Root cause:** The deployment used the wrong `agiToken` address, so escrow and payouts are routed to the wrong token contract.
...
1. **Token selection:** confirm `agiToken` is a standard ERC‑20 (no fee‑on‑transfer, no rebasing) and is correct before deploying (it cannot be changed later).
...
2. **Fix misconfiguration:**
   ...
   - `agiToken` is immutable post‑deploy; if incorrect, redeploy instead of attempting an on-chain change.
...
- `agiToken` is immutable; enforce correctness at deploy time instead of relying on post-deploy updates.
