...
| `additionalAgentPayoutPercentage` | Owner (`setAdditionalAgentPayoutPercentage`) | Fallback payout % for additional agents without AGI-type NFTs. | `1..100`; **enforced with** `max(agent payout %, additionalAgentPayoutPercentage) + validationRewardPercentage <= 100` | Keep ≤ `100 - validationRewardPercentage` and not above your max AGI type payout. | If set too high, settlement can revert for additional agents. |
