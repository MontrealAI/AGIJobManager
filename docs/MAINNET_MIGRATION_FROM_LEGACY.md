...
- `ETHERSCAN_API_KEY` is required for deterministic replay (`txlistinternal` support).
- Optional escape hatch for non-production diagnostics only: `ALLOW_UNSAFE_HTML_FALLBACK=1` (may miss historical internal admin calls).
