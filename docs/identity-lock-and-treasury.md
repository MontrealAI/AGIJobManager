...
## Business-operated model

AGIJobManager is an operator-run escrow and settlement system: the owner retains operational control (pausing, parameter updates, allowlist maintenance, and incident response) while user funds in escrow are protected by on-chain accounting. The contract is intentionally **not** a decentralized governance system; instead, it enforces strong safety invariants around escrow and identity wiring while allowing operational controls for the business operator.
...
This lock is intended to **freeze identity wiring only**, allowing the operator to keep running the system without being able to rewire identity primitives.
