...
The UI verifies the job state (`getJobCore(jobId)` / `getJobValidation(jobId)` / `getJobUris(jobId)`) before sending a transaction and then runs a static-call preflight
...
**range-based pagination** by calling `getJobCore(jobId)` / `getJobValidation(jobId)` / `getJobUris(jobId)` or `ownerOf/tokenURI/listings` only for the
