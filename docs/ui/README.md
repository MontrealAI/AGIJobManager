...
The UI verifies the job state (`getJobCore(jobId)` + `getJobMeta(jobId)`) before sending a transaction and then runs a static-call preflight
...
**range-based pagination** by calling `getJobCore(jobId)` / `getJobMeta(jobId)` or `ownerOf/tokenURI/listings` only for the
current page range (`latest` descending). Filters require the indexer and are disabled otherwise.

### Known indexing limitations

- `JobCreated` does not include the employer address, so job rows hydrate via `getJobCore(jobId)` / `getJobMeta(jobId)`.
