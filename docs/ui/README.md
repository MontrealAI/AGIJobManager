...
USDC entry: [v1.6.1 standalone console](https://github.com/MontrealAI/AGIJobManager/releases/download/v1.9.0/agijobmanager-usdc.html) and current [repository console](../../ui/agijobmanager-usdc.html). The v1.6.1 console includes privacy reviews, explicit draft saving and credential-safe uploads. See [release versus current-source scope](../V1_RELEASE_SCOPE.md#published-download-versus-current-source) for the deployment-tooling changes included in v1.6.1. Voting requires the `getJobBonds` getter, absent from v0.9.5 and older managers. Read the [compatibility notes and earlier bond-quote limitation](../qualification/BUYER_ECONOMICS_FOLLOWUP.md). Verify checksums and configure a verified compatible v0.9.6/v0.9.7 or v1.0.x/v1.6.1 USDC manager; no live manager is supplied by default.
