...
The UI verifies the job core/validation getters (`getJobCore(jobId)` / `getJobValidation(jobId)`) before sending a transaction and then runs a static-call preflight
...
**range-based pagination** by calling `getJobCore(jobId)` / `getJobValidation(jobId)` or `ownerOf/tokenURI/listings` only for the
