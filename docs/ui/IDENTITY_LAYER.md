...
- ENSJobPages: deployment and manager wiring required; no official v0.9.3 address is configured.
- Connected AGIJobManager: a newly deployed and verified v0.9.3 USDC manager (required; no default).
- Active registry: [`config/usdc-deployment.json`](../../config/usdc-deployment.json), currently `deployment-required`.
- Root namespace: explicitly configured dedicated USDC root; fork-rehearsed proposal `usdc-v093.alpha.jobs.agi.eth` (not a live deployment).
...
The historical ENSJobPages v0.2.0 address `0xc19A84D10ed28c2642EfDA532eC7f3dD88E5ed94` and baseline block `24531331` are legacy references, not a verified v0.9.3 deployment. Follow the [replacement and wiring guide](../DEPLOYMENT/ENS_JOB_PAGES_MAINNET_REPLACEMENT.md) before configuring an identity deployment.
...
```json
{
  "chainId": 1,
  "jobId": 42,
  "name": "agijob42.usdc-v093.alpha.jobs.agi.eth",
  "resolver": "0x...",
  "records": {
    "contenthash": "ipfs://...",
    "url": "https://..."
