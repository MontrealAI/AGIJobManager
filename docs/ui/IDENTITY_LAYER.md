...
- ENSJobPages: deployment and manager wiring required; no official v0.9.7 address is configured.
- Connected AGIJobManager: a newly deployed and verified v0.9.7 USDC manager (required; no default).
...
The historical ENSJobPages v0.2.0 address `0xc19A84D10ed28c2642EfDA532eC7f3dD88E5ed94` and baseline block `24531331` are legacy references, not a verified v0.9.7 deployment. Follow the [replacement and wiring guide](../DEPLOYMENT/ENS_JOB_PAGES_MAINNET_REPLACEMENT.md) before configuring an identity deployment.
