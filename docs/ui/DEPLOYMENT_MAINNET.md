...
AGIJobManager v1.0.4. Status: **deployment-required**.

Software release only; no live USDC manager is recorded here. v1.0.4 revises legal notices and console acknowledgement without changing v1.0.3 contract behavior, ABI, bytecode, disabled NFT default or eight fixed library links. Existing jobs and agreements stay with their original deployment. Verify accepted ownership, both recipients, ENS wiring and READINESS_NFT_CONFIG before intake. Initial activation requires zero escrow, bonds and claims. Later recipient/NFT collection changes require zero live escrow and bonds; old claims retain their beneficiary. Complete the deployment-specific operator notice and applicable legal review; technical checks do not certify legal status.
