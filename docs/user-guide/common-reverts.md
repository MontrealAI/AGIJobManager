...
| Apply for a job | `IneligibleAgentPayout` | Your agent payout tier is 0%, so the job cannot be accepted. | Hold an eligible AGI‑type NFT; `additionalAgents` only bypass identity checks and does not grant payout eligibility. | [Roles → Agent](roles.md#agent) |
